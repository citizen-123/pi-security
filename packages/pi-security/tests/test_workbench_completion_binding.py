...
def test_completion_populates_workbench_owned_unsealed_envelope(tmp_path: Path) -> None:
    ...
    findings["findings"][0]["fingerprints"] = {
        ...
        "primary": "pi-security/v0:sha256:wrong",
    }
    ...
    assert sealed_manifest["documentType"] == "pi-security.scan-manifest"
    ...
    assert sealed_findings["documentType"] == "pi-security.findings"
    ...
    assert sealed_findings["findings"][0]["fingerprints"]["algorithm"] == "pi-security/v1"
    assert sealed_coverage["documentType"] == "pi-security.coverage"
...
def test_deep_completion_preserves_running_scan_after_transient_report_failure(
    ...
    (hook_dir / "sitecustomize.py").write_text(
        ...
        "    if name == 'pi_security_report_projection':\n"
...
def test_completion_succeeds_when_all_findings_are_malformed(tmp_path: Path) -> None:
    ...
    sarif = json.loads((scan_dir / "exports/results.sarif").read_text())
    run = sarif["runs"][0]
    assert run["properties"]["piSecurityCoverageCompleteness"] == "partial"
