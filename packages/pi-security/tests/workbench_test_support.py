...
PACKAGE_MANIFEST = Path(__file__).resolve().parents[1] / "package.json"
...
def directory_snapshot_digest(target: Path, *, excluded: tuple[Path, ...] = ()) -> str:
    ...
    update_digest_field(digest, b"format", b"pi-security-directory/v1")
    ...
    return f"pi-security-snapshot/v1:sha256:{digest.hexdigest()}"
    ...
    *,
    ...
    target_contract = {
        ...
        or (
            ...
            else f"pi-security-snapshot/v1:sha256:{'a' * 64}"
        ),
    }
