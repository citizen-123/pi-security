...
def budget_scan_fixture(
    ...
    *,
    ...
    run_workbench(
        ...
        "--scan-id",
        scan_id,
        environment={"PI_HOME": str(tmp_path / "pi-home")},
    )
...
def test_read_only_preflight_missing_state_is_typed_without_creating_directory(
    tmp_path: Path,
) -> None:
    commands = (
        ("get-workspace", "--workspace-id", str(uuid.uuid4())),
        (
            "preflight-deep-scan",
            "--scan-id",
            str(uuid.uuid4()),
    ...
    for index, command in enumerate(commands):
        state_dir = tmp_path / f"missing-parent-{index}" / "state"
        rejected = run_workbench(
            state_dir,
            *command,
            environment={"PI_SECURITY_WORKBENCH_READ_ONLY_PREFLIGHT": "1"},
            check=False,
        )

        assert rejected["returncode"] == 66
        diagnostic = json.loads(str(rejected["stderr"]))
        assert diagnostic == {
            "schemaVersion": 1,
...
def test_start_scan_rejects_changed_preflight_binding_without_scan(
    tmp_path: Path,
) -> None:
    state_dir = tmp_path / "state"
    ...
    workspace = create_saved_workspace(state_dir, first_target, mode="standard")
    snapshot = run_workbench(
        state_dir,
        "get-workspace",
        "--workspace-id",
        str(workspace["id"]),
    ...
    assert snapshot == {
        "id": workspace["id"],
        "mode": "standard",
        "targetPath": str(first_target.resolve()),
    }
    run_workbench(
        state_dir,
        "save-workspace",
    ...
    diagnostic = json.loads(str(rejected["stderr"]))
    assert diagnostic["code"] == "PI_SECURITY_SETUP_CHANGED"
    assert diagnostic["category"] == "setup_changed"
    assert diagnostic["retryable"] is True
    with sqlite3.connect(state_dir / "workbench.sqlite3") as connection:
        assert connection.execute("SELECT COUNT(*) FROM scans").fetchone() == (0,)
    assert not scan_root.exists()
def test_repeat_start_returns_active_scan_when_target_disappeared(
    tmp_path: Path,
) -> None:
    state_dir = tmp_path / "state"
    target = tmp_path / "target"
    ...
    started = run_workbench(state_dir, *start_args)
    target.rmdir()

    repeated = run_workbench(state_dir, *start_args)

    assert repeated["results"]["scanId"] == started["results"]["scanId"]
...
def test_generated_scan_roots_are_private_without_chmodding_configured_roots(
    ...
    private_started = run_workbench(
        ...
        "start-scan",
        "--workspace-id",
        str(private_workspace["id"]),
        "--expected-mode",
        "standard",
        "--expected-target-path",
        str(private_target.resolve()),
        "--scan-root",
        str(private_root),
        "--private-scan-root",
    )
    private_target_root = private_root / private_target.name
    private_scan_dir = Path(str(private_started["results"]["scanDir"]))
    assert private_root.is_dir()
    assert private_target_root.is_dir()
    assert private_scan_dir.is_dir()
    if os.name != "nt":
    ...
    run_workbench(
        ...
        "start-scan",
        "--workspace-id",
        str(configured_workspace["id"]),
        "--expected-mode",
        "standard",
        "--expected-target-path",
        str(configured_target.resolve()),
        "--scan-root",
        str(configured_root),
    )
...
def test_filesystem_identity_serialization_supports_windows_stat_values() -> None:
    namespace = runpy.run_path(str(SCRIPT), run_name="pi_security_workbench_db")
...
def test_workbench_rejects_working_tree_target_after_contents_change(tmp_path: Path) -> None:
    ...
    assert str(saved["diffTarget"]["contentDigest"]).startswith(
        "pi-security-snapshot/v1:sha256:"
    )
