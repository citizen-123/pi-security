...
const bundle = await build({
  ...
  stdin: {
    contents: [
      'export * from "./src/artifact-inventory.ts";',
      'export { createExecutionPolicyContext } from "./src/execution-policy.ts";',
      'export { resolvePythonCommand } from "./src/python_command.ts";'
    ].join("\n"),
    loader: "ts",
    resolveDir: packageRoot,
    sourcefile: "artifact-inventory-test-entry.ts"
  },
  format: "esm",
  platform: "node",
  write: false
...
async function testPrepareUsesTheExistingStandardGenerator() {
  ...
  const result = await inventory.preparePiSecurityReviewItems(fixture.scan);
  ...
  const first = await inventory.listPiSecurityReviewItems(fixture.scan, { limit: 2 });
  ...
  const second = await inventory.listPiSecurityReviewItems(
    fixture.scan,
...
async function testPrepareUsesOnlyAuthoritativeDiffChanges() {
  ...
  assert.deepEqual(await inventory.preparePiSecurityReviewItems(context), {
    reviewItemsTotal: 3
  });
  assert.deepEqual(await inventory.listPiSecurityReviewItems(context), {
    items: [
      { path: "src/changed.ts" },
      { path: "src/deleted.ts" },
      { path: "src/new.ts" }
...
async function testWorkerReadsItsOwnBoundInventory() {
  ...
  assert.deepEqual(
    await inventory.listPiSecurityReviewItems(fixture.scan),
    { items: [{ path: "./src/parent.ts" }] }
  );
  assert.deepEqual(
    await inventory.listPiSecurityReviewItems(fixture.worker),
...
async function testCursorAndLimitAreValidated() {
  ...
  await assert.rejects(
    inventory.listPiSecurityReviewItems(fixture.scan, { cursor: "-1" }),
    /cursor/i
  );
  await assert.rejects(
    inventory.listPiSecurityReviewItems(fixture.scan, { cursor: "3" }),
    /cursor/i
  );
  await assert.rejects(
    inventory.listPiSecurityReviewItems(fixture.scan, { limit: 0 }),
    /limit/i
  );
  await assert.rejects(
    inventory.listPiSecurityReviewItems(fixture.scan, { limit: 1001 }),
    /limit/i
  );
}
...
async function testEmptyInventoryIsValid() {
  ...
  assert.deepEqual(
    await inventory.preparePiSecurityReviewItems(fixture.scan),
    { reviewItemsTotal: 0 }
  );
  assert.equal(await readFile(fixture.scanInventory, "utf8"), "");
  assert.deepEqual(await inventory.listPiSecurityReviewItems(fixture.scan), { items: [] });
}
...
async function testUnsafeInventoryRowsAreRejected() {
  for (const unsafe of ["../outside.ts", "/absolute.ts", "src/../outside.ts", "src\\file.ts"]) {
    ...
    await assert.rejects(
      inventory.listPiSecurityReviewItems(fixture.scan),
...
async function testWorkersCannotPrepareInventory() {
  ...
  await assert.rejects(
    inventory.preparePiSecurityReviewItems(fixture.worker),
...
async function testBoundScopeFailurePreservesPreviousInventory() {
  ...
  await assert.rejects(
    inventory.preparePiSecurityReviewItems(invalid),
...
async function testInvalidDiffTargetPreservesPreviousInventory() {
  ...
  for (const targetContract of [undefined, { diffTarget: { kind: "range" } }]) {
    await assert.rejects(
      inventory.preparePiSecurityReviewItems({
        ...fixture.scan,
        mode: "diff",
        targetContract
      }),
...
async function createFixture(label) {
  ...
  const scanId = "f84c8312-a602-4660-8e01-518a176cd75a";
  const scanExecutionPolicy = inventory.createExecutionPolicyContext({
    profile: "security-artifact-writer",
    target: { root: repoRoot },
    scan: { id: scanId, artifactRoot: scanRoot },
  });
  const workerExecutionPolicy = inventory.createExecutionPolicyContext({
    profile: "security-artifact-writer",
    target: { root: repoRoot },
  ...
  return {
    ...
    scan: {
      ...
      scope: ".",
      packageRoot,
      pythonCommand: await inventory.resolvePythonCommand(),
      executionPolicy: scanExecutionPolicy,
    },
    worker: {
      root: workerRoot,
      repoRoot,
