...
const bundled = await build({
  ...
  stdin: {
    contents: [
      'export * from "./src/artifact-deep-reducer.ts";',
...
const root = await realpath(
  await mkdtemp(path.join(tmpdir(), "pi-security-deep-reducer-"))
);
try {
  ...
  const context = reducerContext(
    outputRoot,
    repoRoot,
    { scanRoot, claimedWorkers: [first, second] }
  );

  const inputs = await getPiSecurityDeepReducerInputs(context);
  await assert.rejects(
    recordPiSecurityDeepReduction(context, draft([shared])),
    /unaccounted|discarded.*finding/,
    "a successful reduction must account for every fresh finding, not just one",
  );
  await assert.rejects(
    recordPiSecurityDeepReduction(context, draft([shared, independent, finding("invented", "src/unreviewed.ts")])),
  ...
  await assert.rejects(
    recordPiSecurityDeepReduction(context, invalidCoverage),
  ...
  await assert.rejects(
    recordPiSecurityDeepReduction(context, draft([])),
  ...
  const outcome = await recordPiSecurityDeepReduction(context, merged);
  ...
  const coverageContext = reducerContext(
    coverageRoot,
    repoRoot,
    { scanRoot, claimedWorkers: [coverageWorker] }
  );
  await recordPiSecurityDeepReduction(coverageContext, draft([]));
  assert.deepEqual(
  ...
  const collisionContext = reducerContext(
    collisionRoot,
    repoRoot,
    { scanRoot, claimedWorkers: [collisionWorker] }
  );
  await assert.rejects(recordPiSecurityDeepReduction(collisionContext, draft([shared])), /ambiguous|unaccounted/);
  const collisionInputs = await getPiSecurityDeepReducerInputs(collisionContext);
  ...
  await recordPiSecurityDeepReduction(collisionContext, draft([{
  ...
  await assert.rejects(recordPiSecurityDeepReduction(collisionContext, draft([{
  ...
  const nextContext = reducerContext(
    nextOutputRoot,
    repoRoot,
    {
      scanRoot,
  ...
  const nextInputs = await getPiSecurityDeepReducerInputs(nextContext);
  assert.deepEqual(nextInputs, {
    discoveries: [{ workerId: third.id, result: withSourceRefs(third) }],
    previous: mergedWithSources
  });
  await assert.rejects(
    recordPiSecurityDeepReduction(nextContext, draft([shared])),
    (error) => error.code === "merge_traceability_unstable_candidate_id"
  );
  assert.deepEqual(
    await recordPiSecurityDeepReduction(nextContext, merged),
  ...
  await assert.rejects(
    getPiSecurityDeepReducerInputs({ root, repoRoot: root, layout: "scan" }),
    /No active Deep reducer is bound/
  );
  await assert.rejects(
    recordPiSecurityDeepReduction(context, {
      ...merged,
  ...
  await assert.rejects(
    getPiSecurityDeepReducerInputs(context),
...
function reducerContext(root, repoRoot, deepReducer) {
  return {
    root,
    repoRoot,
    scanId,
...
function finding(id, repositoryPath) {
  return {
    ...
    provenance: { source: "local_package" }
  };
}
