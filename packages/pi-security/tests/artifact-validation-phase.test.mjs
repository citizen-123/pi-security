...
const result = await build({
  ...
  stdin: {
    contents: [
      'export * from "./src/artifact-validation-phase.ts";',
...
const root = await realpath(await mkdtemp(path.join(tmpdir(), "pi-security-validation-phase-")));
try {
  ...
  const recorded = await recordPiSecurityCandidateValidations(context, {
    validations: updates
  });
  ...
  await recordPiSecurityCandidateValidations(context, { validations: updates });
  assert.deepEqual(await readJsonl(ledger), expected);
  ...
  assert.deepEqual(await recordPiSecurityCandidateValidations(empty, {
    validations: []
  }), {
    kind: "candidate_validations",
    operation: "replace",
  ...
  if (process.platform !== "win32") {
    ...
    await assert.rejects(
      recordPiSecurityCandidateValidations(context, { validations: updates }),
...
async function assertNoMutation(context, ledger, input, expectedError) {
  ...
  await assert.rejects(
    recordPiSecurityCandidateValidations(context, input),
    expectedError
  );
