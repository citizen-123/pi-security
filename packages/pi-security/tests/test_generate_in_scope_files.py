...
def standard_inventory(repository: Path, scope: str) -> bytes:
    selected = repository if scope == "." else repository / scope.removeprefix("./")
    candidates = [selected] if selected.is_file() else selected.rglob("*")
    prefix = "./" if scope == "." or scope.startswith("./") else ""
    rows = []
    for candidate in candidates:
        relative = candidate.relative_to(repository)
        if (
            ".git" in relative.parts
            or relative.parts[0] == "ignored"
            or relative.name.endswith(".skip")
            or candidate.is_symlink()
            or not candidate.is_file()
        ):
            continue
        rows.append(f"{prefix}{relative.as_posix()}\n".encode())
...
def test_inventory_matches_the_deterministic_standard_inventory(
...
def test_inventory_always_uses_forward_slashes(tmp_path: Path) -> None:
    repository = make_repository(tmp_path)
    output = tmp_path / "in_scope_files.txt"
...
def test_inventory_does_not_invoke_an_available_ripgrep_shim(tmp_path: Path) -> None:
    repository = make_repository(tmp_path)
    output = tmp_path / "in_scope_files.txt"
    tools = tmp_path / "tools"
    tools.mkdir()
    if os.name == "nt":
        ripgrep = tools / "rg.cmd"
        ripgrep.write_text("@echo off\r\nexit /b 91\r\n", encoding="utf-8")
    else:
        ripgrep = tools / "rg"
    ...
    assert result.returncode == 0, result.stderr
    assert "./app/routes.py" in output.read_text(encoding="utf-8").splitlines()
