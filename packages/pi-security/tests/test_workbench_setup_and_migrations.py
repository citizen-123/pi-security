...
def test_windows_completion_lock_retries_and_unlocks(tmp_path: Path) -> None:
    ...
    with (
        ...
        mock.patch.dict(os.environ, {"PI_SECURITY_STATE_DIR": str(tmp_path)}),
    ):
        try:
...
def test_workbench_does_not_run_textconv_during_diff_setup(tmp_path: Path) -> None:
    ...
    configure_git_command(target, "diff.pi-security-test.textconv", helper)
    (target / ".gitattributes").write_text("README.md diff=pi-security-test\n")
...
def test_workbench_defaults_to_persistent_pi_home_state(tmp_path: Path) -> None:
    pi_home = tmp_path / "pi-home"
    completed = subprocess.run(
        [sys.executable, str(SCRIPT), "database-info"],
        check=True,
        capture_output=True,
        env={**os.environ, "PI_HOME": str(pi_home), "PI_SECURITY_STATE_DIR": ""},
        text=True,
    )
    assert json.loads(completed.stdout) == {
        "databasePath": str(pi_home / "security" / "workbench.sqlite3")
    }
...
def test_workbench_upgrades_stable_scan_target_identity_migration(tmp_path: Path) -> None:
    ...
    namespace = runpy.run_path(str(SCRIPT), run_name="pi_security_workbench_schema")
...
def test_workbench_reconciles_legacy_execution_profile_migrations(
    ...
    migrations = namespace["MIGRATIONS"]
    if dynamic:
        legacy_model = "pi-next/security-pro"
