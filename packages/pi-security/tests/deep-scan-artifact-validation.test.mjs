...
const {
  ...
  validateReducerArtifacts: validateReducerArtifactsWithPolicy,
} = await import(
...
async function validateReducerArtifacts(input, expectedScanId) {
  const executionPolicy = createExecutionPolicyContext({
    profile: "security-artifact-writer",
    target: { root: repositoryRoot },
    scan: { id: expectedScanId ?? scanId, artifactRoot: input.artifactDir },
  });
  const artifactContext = await createWorkerArtifactContext({
    root: input.artifactDir,
    repoRoot: repositoryRoot,
    layout: "reducer",
    scanId: expectedScanId ?? scanId,
    executionPolicy,
  });
  return await validateReducerArtifactsWithPolicy({
    ...input,
    artifactContext,
  }, expectedScanId);
}
...
async function testReducerValidation(root) {
  ...
  await writeResult(resultPath, draft([
    {
      ...collidingOriginalA,
      provenance: { source: "local_package", sourceFindingIds: ["origin:a"] },
    },
    {
      ...collidingOriginalB,
      provenance: { source: "local_package", sourceFindingIds: ["origin:b"] },
    },
  ]));
  await validateReducerArtifacts({
    artifacts,
