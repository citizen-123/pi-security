...
async function testCappedQueueAndSerialDedup() {
  ...
  for (const worker of [...store.workers.values()].filter((worker) => (
    ...
    assert.equal(context.packageRoot, fixture.packageRoot);
  }
...
async function testConsumedSourceWithToolDiagnosticIsNotRereadAfterReducerWritesResult() {
  ...
  const executor = new FakeExecutor({
    ...
    dedupDiagnostics: [{
      ...
      message: "Pi worker artifact tool record_pi_security_deep_reduction failed."
    }]
  });
  const coordinator = new DeepScanCoordinator({
    run: fixture.run,
...
async function testTransientExecutionFailureResumesWorkerThread() {
  ...
  assert.match(executor.discoveryContinuationPrompts[1] ?? "", /record_pi_security_scan_draft/);
...
async function testInvalidArtifactsRetry() {
  ...
  assert.match(retryInstructions, /record_pi_security_scan_draft/);
...
async function testRegistryEvictionAndExternalFailure() {
  ...
  const completed = registry.start({
    ...
    packageRoot: completedFixture.packageRoot,
    clock: immediateClock
  });
  ...
  const failed = registry.start({
    ...
    packageRoot: failedFixture.packageRoot,
    clock: immediateClock
  });
...
async function testPolicyFailureIdentitySurvivesPublicationWithoutPrefixSpoofing() {
  const runCase = async (discoveryError) => {
    const fixture = await fixtureRun({
      workers: 1,
      subagents: 0,
      stopAfterNoNew: 1,
    ...
    const store = new FakeStore(fixture.run);
    const executor = new FakeExecutor({ discoveryError });
    const coordinator = new DeepScanCoordinator({
      run: fixture.run,
      store,
      executor,
      packageRoot: fixture.packageRoot,
      clock: immediateClock,
      threadId: "policy-publication-owner",
      onStopped: async () => {
        throw new Error("fixture policy result publication failure");
      },
    });
    coordinator.start();
    return {
      terminal: await coordinator.wait(undefined, 5_000),
      store,
    };
  };

  const trusted = await runCase(new PolicyRecoveryRejectedError(
    "binding_mismatch",
    "fixture authoritative continuation binding mismatch",
  ));
  assert.deepEqual(trusted.terminal?.policyFailure, {
    schemaVersion: 1,
    code: "PI_SECURITY_POLICY_RECOVERY_REJECTED",
    category: "policy_recovery_rejected",
    reason: "binding_mismatch",
    message: "PI_SECURITY_POLICY_RECOVERY_REJECTED: fixture authoritative continuation binding mismatch",
  });
  assert.deepEqual(
    trusted.store.failureInputs[0].policyFailure,
    trusted.terminal?.policyFailure,
  );
  const atomicWorkerFailure = trusted.store.workerUpdates.find((update) => (
    update.status === "failed" && update.policyFailure
  ));
  assert.deepEqual(
    atomicWorkerFailure?.policyFailure,
    trusted.terminal?.policyFailure,
    "the worker failure mutation must carry the trusted identity before coordinator failure",
  );
  assert.match(
    trusted.terminal?.error ?? "",
    /Saved result publication failed: fixture policy result publication failure/u,
  );
  assert.match(trusted.terminal?.error ?? "", /Original Deep Scan failure:/u);

  const spoof = Object.assign(
    new Error("PI_SECURITY_POLICY_RECOVERY_REJECTED: spoofed transport failure"),
    { code: "ENOENT" },
  );
  const untrusted = await runCase(spoof);
  assert.equal(untrusted.terminal?.policyFailure, undefined);
  assert.equal(
    Object.hasOwn(untrusted.store.failureInputs[0], "policyFailure"),
    false,
  );
  assert.match(
    untrusted.terminal?.error ?? "",
    /Saved result publication failed: fixture policy result publication failure/u,
  );
}
async function testCrashWindowRejoinStopsAtPersistedWorkerPolicyFailure() {
  const fixture = await fixtureRun({
    workers: 1,
    subagents: 0,
    stopAfterNoNew: 1,
  ...
  const policyFailure = {
    ...
    category: "policy_recovery_rejected",
    reason: "binding_mismatch",
    message: "PI_SECURITY_POLICY_RECOVERY_REJECTED: persisted crash-window binding mismatch",
  };
  const failedWorker = {
    id: randomUUID(),
    kind: "discovery",
  ...
  const store = new FakeStore(crashedRun);
  const executor = new FakeExecutor();
  await rm(fixture.run.scanDir, { recursive: true });
  const coordinator = new DeepScanCoordinator({
    run: crashedRun,
    store,
    executor,
  ...
  const terminal = await coordinator.wait(undefined, 5_000);
  assert.equal(terminal?.status, "failed");
  assert.deepEqual(terminal?.policyFailure, policyFailure);
  assert.deepEqual(store.failureInputs, [{
    message: policyFailure.message,
    status: "failed",
    manifestPath: undefined,
    policyFailure,
  }]);
  assert.equal(executor.calls, 0, "rejoin must not sample after an atomic worker policy failure");
  assert.equal(store.heartbeatCalls, 0, "rejoin must not schedule coordinator heartbeats");
  await assert.rejects(readdir(fixture.run.scanDir), { code: "ENOENT" });
}
...
async function testForgedCrashResumeRejectsBeforeAdoptionReplacement() {
  const fixture = await fixtureRun({
    workers: 1,
    subagents: 1,
    stopAfterNoNew: 2,
  ...
  const workerRoot = path.join(fixture.run.scanDir, "workers", "discovery-0001");
  ...
  let replacements = 0;
  let validations = 0;
  const policyFailure = new PolicyRecoveryRejectedError(
    "profile_mismatch",
    "fixture forged resumed continuation",
  );

  await assert.rejects(
    startOrJoinDeepScanCoordinator({
      begin: { run: running, startDisposition: "joined" },
      registry: {
        get: () => undefined,
        start: () => {
          replacements += 1;
      ...
      options: {
        store: {
          async claimCoordinator() {
            claims += 1;
            throw new Error("claim would mutate durable adoption state");
          },
        },
        executor: new FakeExecutor({
  ...
  assert.equal(claims, 0, "forged continuation must reject before durable adoption");
  assert.equal(replacements, 0, "forged continuation must not dispatch a replacement");
  assert.equal(validations, 1, "authorized resumable paths must reach policy validation");
  assert.deepEqual(
    Buffer.from(JSON.stringify(running)),
    durableBefore,
    "failed recovery validation must leave durable run bytes unchanged",
  );

  const outsideRoot = await mkdtemp(path.join(tmpdir(), "pi-security-forged-resume-root-"));
  try {
    const outsideArtifactDir = path.join(outsideRoot, "output");
    await mkdir(outsideArtifactDir);
    const forgedPathRun = {
      ...running,
      persistedWorkers: [{
        ...running.persistedWorkers[0],
        promptPath: path.join(outsideRoot, "prompt.md"),
        artifactDir: outsideArtifactDir,
      }],
    };
    let forgedPathValidations = 0;
    await assert.rejects(
      startOrJoinDeepScanCoordinator({
        begin: { run: forgedPathRun, startDisposition: "joined" },
        registry: {
          get: () => undefined,
          start: () => {
            replacements += 1;
            throw new Error("forged-path replacement must not start");
          },
        },
        options: {
          store: {
            async claimCoordinator() {
              claims += 1;
              throw new Error("forged-path claim must not run");
            },
          },
          executor: new FakeExecutor({
            async validateContinuationPolicy() {
              forgedPathValidations += 1;
            },
          }),
          packageRoot: fixture.packageRoot,
          threadId: "thread-fixture",
    ...
    assert.equal(forgedPathValidations, 0);
...
async function fixtureRun(config) {
  const root = await realpath(
    await mkdtemp(path.join(tmpdir(), "pi-security-deep-coordinator-"))
  );
  temporaryRoots.push(root);
  const targetPath = path.join(root, "target");
  const scanDir = path.join(root, "scan");
  const packageRoot = path.join(root, "package");
  await Promise.all([mkdir(targetPath), mkdir(scanDir), mkdir(packageRoot)]);
  return {
    packageRoot,
    run: {
      scanId: randomUUID(),
...
class FakeStore {
  ...
  async updateWorker(update) {
    ...
    if (update.policyFailure) {
      if (this.run.policyFailure) {
        assert.deepEqual(this.run.policyFailure, update.policyFailure);
      }
      this.run.policyFailure = structuredClone(update.policyFailure);
    }
  ...
  async fail(
    ...
    this.run.policyFailure = policyFailure;
...
class FakeExecutor {
  ...
  async validateContinuationPolicy(request) {
    await this.options.validateContinuationPolicy?.(request);
  }
  ...
  async run(request) {
    ...
    if (request.kind === "discovery") {
      ...
      try {
        if (this.options.discoveryError) throw this.options.discoveryError;
...
function standardScanDraft(scanId, candidateId, workerLabel) {
  return {
    ...
    findings: candidateId ? [{
      ...
      provenance: { source: "local_package", candidateId, workerId: workerLabel }
    }] : [],
    coverage: {
...
try {
  ...
  await testPolicyFailureIdentitySurvivesPublicationWithoutPrefixSpoofing();
  await testCrashWindowRejoinStopsAtPersistedWorkerPolicyFailure();
  ...
  await testForgedCrashResumeRejectsBeforeAdoptionReplacement();
