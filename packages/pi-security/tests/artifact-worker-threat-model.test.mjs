...
const bundle = await build({
  ...
  stdin: {
    contents: [
      'export * from "./src/artifact-threat-model.ts";',
...
const fixtureRoot = await realpath(
  await mkdtemp(path.join(tmpdir(), "pi-security-worker-threat-model-"))
);
...
console.log("Pi Security worker threat-model artifact tests passed");

async function testCheckedInStrictSchema() {
  assert.equal(schema.$schema, "https://json-schema.org/draft/2020-12/schema");
  assert.equal(
    schema.$id,
    "pi-security://schemas/tools/worker-threat-model.schema.json"
  );
...
async function testExactContentAndAtomicReplacement() {
  ...
  assert.deepEqual(
    await threatModel.recordPiSecurityWorkerThreatModel(
      { content: original },
      context
    ),
    { operation: "replace" }
...
async function testInvalidInputDoesNotWrite() {
  for (const [index, input] of [
    ...
    await assert.rejects(
      threatModel.recordPiSecurityWorkerThreatModel(input, context)
    );
    await assert.rejects(
...
async function testOnlyDiscoveryWorkersCanWrite() {
  for (const layout of ["scan", "reducer"]) {
    ...
    await assert.rejects(
      threatModel.recordPiSecurityWorkerThreatModel(
...
async function createWorkerContext(name) {
  ...
  return {
    ...
    layout: "worker",
    scanId,
    executionPolicy: threatModel.createExecutionPolicyContext({
      profile: "security-artifact-writer",
      target: { root: repoRoot },
      scan: { id: scanId, artifactRoot: canonicalRoot },
    }),
  };
}
