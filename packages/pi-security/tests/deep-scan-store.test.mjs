...
const bundle = await build({
  ...
  stdin: {
    contents: [
      'export * from "./src/deep-scan/store.ts";',
...
const unitExecutionContext = createExecutionPolicyContext({
  profile: "security-artifact-writer",
  target: { root: "/" },
  scan: { id: "deep-scan-store-unit", artifactRoot: "/" },
});

function createStore(runWorkbench, preflightWorkbench = runWorkbench) {
  return new WorkbenchDeepScanStore(
    runWorkbench,
    async () => unitExecutionContext,
    preflightWorkbench,
  );
}

await testBeginProtocolAndParsing();
await testAuthenticatedPreflightProtocol();
...
async function testAuthenticatedPreflightProtocol() {
  ...
  const store = createStore(
    async (args) => {
      ...
      throw new Error("preflight must not invoke the mutating workbench runner");
    },
    async (args, input) => {
      preflightCalls.push({ args, input });
      return {
        deepScanPreflight: {
          scanId,
          targetPath: "/fixture/repository",
          scope: ".",
          scanDir: "/fixture/scans/repository/scan",
          subagents: 3,
          resumableWorkers: [],
        },
      };
...
async function testBeginProtocolAndParsing() {
  ...
  assert.equal(result.artifactWriteAuthorization, "creator");
  ...
  const joined = await createStore(joinedRunner).begin({
    scanId,
    handoffClaimToken: claimToken,
    threadId: "thread-fixture",
    scanRoot: "/fixture/scans"
  ...
  assert.equal(
    joined.artifactWriteAuthorization,
    "owning_thread_live_rejoin",
  );
  await assert.rejects(
    createStore(async () => stateResult(scanId, {
      startDisposition: "joined",
      artifactWriteAuthorization: "creator",
    })).begin({
      scanId,
      threadId: "foreign-thread",
      scanRoot: "/fixture/scans",
    }),
    /invalid Deep Scan artifact-write authorization/,
  );
...
async function testWorkerResponseParsing() {
  ...
  const worker = await createStore(async () => stateResult(scanId, {
    deepScan: {
      workers: [{
        id: workerId,
        kind: "discovery",
...
async function testPerScanExecutionContextsRemainBound() {
  const [firstRoot, secondRoot] = await Promise.all([
    mkdtemp(join(tmpdir(), "pi-security-store-first-root-")),
    mkdtemp(join(tmpdir(), "pi-security-store-second-root-")),
  ]);
  ...
  const roots = new Map([
    [firstScanId, firstRoot],
    [secondScanId, secondRoot],
  ]);
  const mutation = (scanId) => {
    const root = roots.get(scanId);
    return {
      id: randomUUID(),
      scanId,
      kind: "discovery",
  ...
  const store = new WorkbenchDeepScanStore(
    async (args) => {
      const scanId = flagValue(args, "--scan-id");
      const workerId = flagValue(args, "--worker-id");
      const root = roots.get(scanId);
    ...
    async (scanId) => {
      ...
      const context = executionContexts.get(scanId);
      assert.ok(context, `missing execution context for ${scanId}`);
      return context;
    },
  );

  try {
    executionContexts.set(firstScanId, createExecutionPolicyContext({
      profile: "security-artifact-writer",
      target: { root: "/" },
      scan: { id: firstScanId, artifactRoot: firstRoot },
    }));
    await store.updateWorker(mutation(firstScanId));

    executionContexts.set(secondScanId, createExecutionPolicyContext({
      profile: "security-artifact-writer",
      target: { root: "/" },
      scan: { id: secondScanId, artifactRoot: secondRoot },
    }));
    await store.updateWorker(mutation(secondScanId));
    await store.updateWorker(mutation(firstScanId));
...
async function testAtomicWorkerPolicyFailureProtocol() {
  const scanId = randomUUID();
  const workerId = randomUUID();
  let invocation;
  const policyFailure = {
    ...
    message: "PI_SECURITY_POLICY_RECOVERY_REJECTED: fixture worker binding mismatch",
  };
  const store = createStore(async (args) => {
    invocation = args;
    return stateResult(scanId, {
  ...
  assert.equal(flagValue(invocation, "--policy-failure-code"), policyFailure.code);
  assert.equal(flagValue(invocation, "--policy-failure-category"), policyFailure.category);
  assert.equal(flagValue(invocation, "--policy-failure-reason"), policyFailure.reason);
  assert.equal(flagValue(invocation, "--policy-failure-message"), policyFailure.message);
}
...
async function testTerminalProtocol() {
  ...
  const store = createStore(async (args) => {
    ...
    return stateResult(scanId, {
      deepScan: {
        ...
        policyFailure: args[0] === "fail-deep-scan"
          ? {
              schemaVersion: 1,
              code: flagValue(args, "--policy-failure-code"),
              category: flagValue(args, "--policy-failure-category"),
              reason: flagValue(args, "--policy-failure-reason"),
              message: flagValue(args, "--policy-failure-message"),
            }
          : undefined
      }
    });
  ...
  const failed = await store.fail(
    ...
    undefined,
    {
      ...
      category: "policy_recovery_rejected",
      reason: "binding_mismatch",
      message: "PI_SECURITY_POLICY_RECOVERY_REJECTED: fixture binding mismatch",
    },
  );
  assert.deepEqual(
  ...
  assert.equal(
    flagValue(calls[1], "--policy-failure-code"),
    "PI_SECURITY_POLICY_RECOVERY_REJECTED",
  );
  assert.equal(flagValue(calls[1], "--policy-failure-reason"), "binding_mismatch");
  assert.equal(failed.policyFailure.reason, "binding_mismatch");
}
...
async function testDeterministicPersistenceFailuresAreNotRetried() {
  for (const diagnostic of [
    ...
    "Authentication required for Pi Security workbench."
  ]) {
    const scenario = idempotentPersistenceScenarios()[1];
  ...
  for (const status of ["queued", "running", "succeeded", "failed", "canceled"]) {
    for (const failure of [
      ...
      const workerStore = createStore(async () => {
        workerAttempts += 1;
        throw failure;
      });
...
function stateResult(scanId, overrides = {}) {
  ...
  const status = deepScanOverride.status ?? "running";
  const defaultAuthorization = overrides.startDisposition === "created"
    ? "creator"
    : overrides.startDisposition === "joined"
      ? status === "running"
        ? "owning_thread_live_rejoin"
        : "none"
      : undefined;
  return {
    ...
    artifactWriteAuthorization:
      overrides.artifactWriteAuthorization ?? defaultAuthorization,
    deepScan: {
      scanId,
      status,
      targetPath: "/fixture/repository",
