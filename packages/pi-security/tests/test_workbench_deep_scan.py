...
def deep_environment(pi_home: Path) -> dict[str, str]:
    return {"PI_HOME": str(pi_home)}
...
def test_existing_generation_safely_claims_and_reclaims_without_schema_migration(
    ...
    target.mkdir()
    scan_id = str(
        begin_target_scan(state_dir, pi_home, target, tmp_path / "scans")["deepScan"]["scanId"]
    )
    ...
    renewed = claim_deep_scan_coordinator(state_dir, pi_home, scan_id, coordinator_generation=2)
...
def test_legacy_generation_with_active_worker_observes_grace_then_adopts(tmp_path: Path) -> None:
    ...
    observed = claim_deep_scan_coordinator(state_dir, pi_home, scan_id)
    ...
    adopted = claim_deep_scan_coordinator(state_dir, pi_home, scan_id)
...
def test_expired_coordinator_preserves_incomplete_setup(tmp_path: Path) -> None:
    ...
    claimed = claim_deep_scan_coordinator(state_dir, pi_home, scan_id)["deepScan"]
    assert (claimed["phase"], claimed["coordinatorGeneration"]) == ("setup", 2)
    assert not inventory.exists()

    expire_deep_scan_coordinator(state_dir, scan_id)
    recovered = claim_deep_scan_coordinator(state_dir, pi_home, scan_id)["deepScan"]
...
def test_paused_discovery_resumes_after_restart_with_original_handoff_claim(
    ...
    state_dir, pi_home, target = tmp_path / "state", tmp_path / "pi-home", tmp_path / "target"
    target.mkdir()
    config = pi_home / "pi-security" / "config.toml"
...
def test_expired_coordinator_recovers_reducer_publication_after_process_death(
    ...
    crashed = subprocess.run(
        ...
        env={
            **os.environ,
            "PI_SECURITY_STATE_DIR": str(state_dir),
            **deep_environment(pi_home),
        },
        capture_output=True,
        text=True,
    )
...
def test_expired_generation_preserves_receipts_and_recovers_abandoned_workers(
    ...
    state_dir, pi_home, target = tmp_path / "state", tmp_path / "pi-home", tmp_path / "target"
    config_path = pi_home / "pi-security" / "config.toml"
...
def test_expired_generation_refunds_shutdown_canceled_discovery(
    ...
    expire_deep_scan_coordinator(state_dir, scan_id)
    assert (
        claim_deep_scan_coordinator(state_dir, pi_home, scan_id)["deepScan"]["dispatchedCount"]
        == 1
    )
...
def test_expired_generation_cannot_mutate_recovered_scan(tmp_path: Path, mutation: str) -> None:
    ...
    expire_deep_scan_coordinator(state_dir, scan_id)
    assert (
        claim_deep_scan_coordinator(state_dir, pi_home, scan_id)["deepScan"][
...
def test_reducer_commit_requires_safe_standard_canonical_artifacts(
    ...
    target.mkdir()
    begun = begin_target_scan(state_dir, pi_home, target, tmp_path / "scans")
...
def test_scan_progress_projects_active_and_completed_independent_reviews(
    ...
    target = tmp_path / "target"
    target.mkdir()
    started = begin_target_scan(state_dir, pi_home, target, tmp_path / "scans")
...
def test_reducer_claim_updates_review_pass_once_and_projects_consolidation(
    ...
    for _ in range(2):
        run_workbench(state_dir, *claim_args, environment=deep_environment(pi_home))
...
def test_deep_scan_prefers_explicit_config_path(tmp_path: Path) -> None:
    state_dir = tmp_path / "state"
    pi_home = tmp_path / "pi-home"
    shared_config_path = pi_home / "pi-security" / "config.toml"
    ...
    deep_scan = run_workbench(
        ...
        environment={
            **deep_environment(pi_home),
            "PI_SECURITY_DEEP_SCAN_CONFIG_PATH": str(isolated_config_path),
        },
    )["deepScan"]
...
def test_target_begin_is_atomic_idempotent_and_snapshots_config(tmp_path: Path) -> None:
    ...
    first = begin_target_scan(state_dir, pi_home, target, scan_root)
    ...
    config_path.write_text("[deep_scan]\nworkers = 1\nmax_time_hours = 8\n")
    second = begin_target_scan(state_dir, pi_home, target, scan_root)
...
def test_sdk_scan_begin_claims_its_existing_scan(tmp_path: Path) -> None:
    ...
    assert begun["artifactWriteAuthorization"] == "creator"
    rejoined = run_workbench(
        state_dir,
        "begin-deep-scan",
        "--thread-id",
        "sdk-thread",
    ...
    assert (
        rejoined["artifactWriteAuthorization"]
    ...
    assert "owning host session" in str(rejected["stderr"])
...
def test_discovery_error_threshold_defaults_to_three_independently_of_no_new_threshold(
    ...
    deep_scan = begin_target_scan(state_dir, pi_home, target, tmp_path / "scans")["deepScan"]
...
def test_failed_reducer_rebuffers_claimed_inputs_for_same_generation_replacement(
    ...
    target = tmp_path / "target"
    target.mkdir()
    begun = begin_target_scan(state_dir, pi_home, target, tmp_path / "scans")["deepScan"]
...
def test_concurrent_target_begin_creates_one_scan(tmp_path: Path) -> None:
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        results = list(
            executor.map(
                lambda _: begin_target_scan(state_dir, pi_home, target, scan_root),
                range(2),
            )
        )
...
def test_app_scan_begin_validates_mode_and_owner(tmp_path: Path) -> None:
    ...
    assert "owning host session" in str(wrong_owner["stderr"])
...
def test_stale_deep_continuation_cannot_begin_after_handoff_transfer(
    ...
    assert "owning host session" in str(stale_thread["stderr"])
...
def prepare_failure_capped_deep_scan(
    ...
    return state_dir, pi_home, scan_dir, scan_id


def test_failure_capped_completion_preserves_partial_results_and_exact_omissions(
    tmp_path: Path,
) -> None:
    state_dir, pi_home, scan_dir, scan_id = prepare_failure_capped_deep_scan(tmp_path)
    buffered_id, *_ = dispatch_discovery_worker(
        state_dir,
        pi_home,
        scan_id=scan_id,
    ...
    without_omission = run_workbench(
        state_dir, *finish_args, environment=deep_environment(pi_home), check=False
    )
...
def test_late_worker_rejection_supersedes_stopped_checkpoint_finding(tmp_path: Path) -> None:
    ...
    recovery_needed = run_workbench(
        state_dir, "get-scan", "--scan-id", scan_id, environment=deep_environment(pi_home)
    )["scan"]
...
def test_failure_capped_completion_recovers_canceled_reducer_inputs(tmp_path: Path) -> None:
    ...
    finished = run_workbench(state_dir, *finish_args, environment=deep_environment(pi_home))[
        "deepScan"
    ]
    assert finished["status"] == "succeeded"
    assert (
...
def test_running_worker_updates_preserve_identity_and_dispatch_count(tmp_path: Path) -> None:
    ...
    initial = upsert_worker(state_dir, pi_home, **update)["deepScan"]
    initial_worker = next(worker for worker in initial["workers"] if worker["id"] == worker_id)
    repeated = upsert_worker(state_dir, pi_home, **update)["deepScan"]
