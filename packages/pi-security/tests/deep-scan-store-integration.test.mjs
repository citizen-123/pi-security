...
const workbenchPath = path.join(packageRoot, "scripts", "workbench_db.py");
...
const bundle = await build({
  ...
  stdin: {
    contents: [
      ...
      'export { recordPiSecurityScanDraftViaWorkbench } from "./src/artifact-scan-draft.ts";',
...
function createStore(runWorkbench, fixtureRoot) {
  const executionContext = createExecutionPolicyContext({
    profile: "security-artifact-writer",
    target: { root: fixtureRoot },
    scan: { id: "deep-scan-store-integration", artifactRoot: fixtureRoot },
  });
...
async function testRecoveredPublicationRejectsLateFailure() {
  ...
  const environment = {
    ...process.env,
    PI_HOME: path.join(fixtureRoot, "home"),
    PI_SECURITY_STATE_DIR: path.join(fixtureRoot, "state"),
  };
  const runWorkbench = async (args) => {
    const { stdout } = await execFileAsync(python, [workbenchPath, ...args], {
      cwd: packageRoot, env: environment, timeout: 30_000, maxBuffer: 4 * 1024 * 1024,
    });
  ...
  try {
    ...
    const store = createStore(runWorkbench, fixtureRoot);
    ...
    const reloaded = await createStore(runWorkbench, fixtureRoot).get(
      run.scanId,
...
async function testNoopStoppedRefreshRetainsPublicationFailure() {
  ...
  try {
    ...
    assert.equal(
      (await createStore(runWorkbench, fixtureRoot).get(
        run.scanId,
...
async function testConcurrentParentDraftsPreserveBothCheckpoints() {
  ...
  try {
    await mkdir(targetPath, { recursive: true });
    await writeFile(path.join(targetPath, "fixture.py"), "print('fixture')\n");
    const { run } = await createStore(rawRunWorkbench, fixtureRoot).begin({
...
async function testLateParentDraftPreservesCheckpointWithoutOverwritingTerminalSeal() {
  ...
  try {
    await mkdir(targetPath, { recursive: true });
    await writeFile(path.join(targetPath, "fixture.py"), "print('fixture')\n");
    const { run } = await createStore(runWorkbench, fixtureRoot).begin({
...
async function testReducerCommitAndFinishAgainstRealWorkbench() {
  ...
  const piHome = path.join(fixtureRoot, "pi-home");
  const threadId = "deep-scan-store-integration-thread";
  const environment = {
    ...process.env,
    PI_HOME: piHome,
    PI_SECURITY_STATE_DIR: stateDir
  };
  const runWorkbench = async (args) => {
    const { stdout } = await execFileAsync(python, [workbenchPath, ...args], {
      cwd: packageRoot,
  ...
  try {
    ...
    await writeFile(path.join(targetPath, "fixture.py"), "print('fixture')\n");
    await writeFile(
      path.join(piHome, "pi-security", "config.toml"),
      [
        "[deep_scan]",
        "workers = 2",
        "subagents = 0",
    ...
    assert.equal(begun.startDisposition, "created");
    assert.equal(begun.artifactWriteAuthorization, "creator");
    const { run } = begun;
    ...
    const observer = createStore(runWorkbench, fixtureRoot);
    ...
    assert.equal(continued.startDisposition, "joined");
    assert.equal(continued.artifactWriteAuthorization, undefined);
...
async function testExpiredDeadlineWithoutCompletedDiscoveryAgainstRealWorkbench() {
  ...
  try {
    ...
    const observed = await createStore(runWorkbench, fixtureRoot).get(run.scanId, threadId);
