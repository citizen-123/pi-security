...
function bindContext(base, artifactRoot) {
  return {
    ...base,
    root: artifactRoot,
    executionPolicy: module.createExecutionPolicyContext({
      profile: "security-artifact-writer",
      target: { root: base.repoRoot },
      scan: { id: base.scanId, artifactRoot },
    }),
  };
}

const root = await realpath(
  await mkdtemp(path.join(tmpdir(), "pi-security-scan-draft-")),
);
const repoRoot = await realpath(
  await mkdtemp(path.join(tmpdir(), "pi-security-scan-draft-repository-")),
);
...
try {
    ...
    targetContract: {
      target: {
        ...
        requiredSnapshotDigest:
          "pi-security-snapshot/v1:sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
      },
      scope: {
  ...
  const workerContext = bindContext({
    repoRoot,
    layout: "worker",
    scanId,
  }, workerRoot);
  ...
  const checkpointContext = bindContext(workerContext, checkpointRoot);
  const checkpoint = { ...workerInput, complete: false };
  await recordPiSecurityWorkerScanDraft(checkpointContext, checkpoint);
  ...
  await recordPiSecurityWorkerScanDraft(checkpointContext, { ...workerInput, findings: [] });
  assert.deepEqual(
  ...
  await recordPiSecurityWorkerScanDraft(interruptedContext, {
    ...workerInput,
    findings: [],
  });
  ...
  const rejectedIncompleteContext = bindContext(workerContext, rejectedIncompleteRoot);
  await recordPiSecurityWorkerScanDraft(rejectedIncompleteContext, workerInput);
  await recordPiSecurityWorkerScanDraft(rejectedIncompleteContext, {
    ...workerInput,
    complete: false,
    findings: [],
    coverage: {
  ...
  const deferredDoesNotRejectContext = bindContext(workerContext, deferredDoesNotRejectRoot);
  await recordPiSecurityWorkerScanDraft(deferredDoesNotRejectContext, workerInput);
  await recordPiSecurityWorkerScanDraft(deferredDoesNotRejectContext, {
    ...workerInput,
    complete: false,
    findings: [],
    coverage: {
  ...
  await recordPiSecurityWorkerScanDraft(checkpointContext, {
    ...workerInput,
    findings: [],
  ...
  const rejectionWithoutNotesContext = bindContext(workerContext, rejectionWithoutNotesRoot);
  await recordPiSecurityWorkerScanDraft(rejectionWithoutNotesContext, checkpoint);
  const { notes: _notes, ...rejectionWithoutNotes } = rejection;
  await recordPiSecurityWorkerScanDraft(rejectionWithoutNotesContext, {
    ...workerInput,
    findings: [],
  ...
  const carriedContext = bindContext(workerContext, carriedContextRoot);
  await recordPiSecurityWorkerScanDraft(carriedContext, { ...workerInput, complete: false });
  const { scope: _scope, threatModel: _threatModel, ...workerWithoutContext } = workerInput;
  await recordPiSecurityWorkerScanDraft(carriedContext, workerWithoutContext);
  ...
  const coverageProgressContext = bindContext(workerContext, coverageProgressRoot);
  const pendingQuestion = "Does the alternate archive handler enforce containment?";
  await recordPiSecurityWorkerScanDraft(coverageProgressContext, {
    ...workerInput,
    complete: false,
    coverage: {
      ...coverage,
  ...
  const renamedProgressContext = bindContext(workerContext, renamedProgressRoot);
  const staleSurfaces = [
    {
  ...
  await recordPiSecurityWorkerScanDraft(renamedProgressContext, {
    ...workerInput,
    complete: false,
    findings: [],
    coverage: {
  ...
  const anchorContext = bindContext(workerContext, anchorRoot);
  ...
  await recordPiSecurityWorkerScanDraft(anchorContext, {
    ...workerInput,
    complete: false,
  ...
  await recordPiSecurityScanDraft(bindContext(context, parentCheckpointRoot), { ...input, complete: false });
  ...
  await recordPiSecurityScanDraft(bindContext(context, parentCheckpointRoot), { ...input, findings: [] });
  ...
  const interruptedParentContext = bindContext(context, interruptedParentRoot);
  ...
  await recordPiSecurityScanDraft(interruptedParentContext, {
    ...input,
    findings: [],
  });
  assert.deepEqual(
  ...
  await recordPiSecurityScanDraft(
    bindContext(context, parentCheckpointRoot),
  ...
  const pendingContext = bindContext(workerContext, pendingRoot);
  ...
  await recordPiSecurityWorkerScanDraft(pendingContext, pending);
  await recordPiSecurityWorkerScanDraft(pendingContext, workerInput);
  ...
  await recordPiSecurityWorkerScanDraft(pendingContext, pending);
  await recordPiSecurityWorkerScanDraft(pendingContext, {
  ...
  const undefinedCandidateContext = bindContext(workerContext, undefinedCandidateRoot);
  await recordPiSecurityWorkerScanDraft(undefinedCandidateContext, {
    ...workerInput,
    complete: false,
    findings: [],
    coverage: {
  ...
  for (const [index, previousFindings] of ["legacy metadata", { opaque: true }, 7].entries()) {
    ...
    const historyContext = bindContext(workerContext, historyRoot);
    const earlierFinding = {
      ...finding,
      summary: "Earlier verified source proof.",
      provenance: { ...finding.provenance, previousFindings },
    };
    await recordPiSecurityWorkerScanDraft(historyContext, { ...workerInput, findings: [earlierFinding] });
    await recordPiSecurityWorkerScanDraft(historyContext, workerInput);
  ...
  await assert.rejects(
    recordPiSecurityWorkerScanDraft(workerContext, {
      ...workerInput,
  ...
  await assert.rejects(readFile(workerResultPath), { code: "ENOENT" });
  assert.deepEqual(
    await recordPiSecurityWorkerScanDraft(workerContext, workerInput),
    {
      scanId,
      findingCount: 1,
      surfaceCount: 1,
  ...
  assert.deepEqual(
    await recordPiSecurityWorkerScanDraft(
  ...
  await assert.rejects(
    recordPiSecurityWorkerScanDraft(context, workerInput),
        ...
        findings: [
          {
            ...finding,
            fingerprints: {
              algorithm: "pi-security/v1",
              primary:
                "pi-security/v1:sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
            },
          },
        ],
      },
  ...
  const retried = await recordPiSecurityScanDraft(
    context,
    input,
    async () => {
      conflictAttempts += 1;
  ...
  const monotonicContext = bindContext(context, monotonicRoot);
  ...
  await recordPiSecurityScanDraftViaWorkbench(
    ...
    async (arguments_) => {
      ...
      if (monotonicWrites === 1) {
        await recordPiSecurityScanDraft(monotonicContext, finalDraft);
  ...
  const archivedRetryContext = bindContext(workerContext, archivedRetryOutput);
  ...
  const archivedResolutionContext = bindContext(workerContext, archivedResolutionOutput);
  await recordPiSecurityWorkerScanDraft(archivedResolutionContext, {
    ...workerInput,
    complete: false,
  });
  ...
  const repeatedCheckpointContext = bindContext(workerContext, repeatedCheckpointOutput);
  ...
  await recordPiSecurityWorkerScanDraft(repeatedCheckpointContext, {
    ...workerInput,
    complete: false,
    findings: [],
    coverage: {
  ...
  const multiAttemptContext = bindContext(workerContext, multiAttemptOutput);
  const multiAttemptArchive = path.join(multiAttemptRoot, "attempts");
  await mkdir(multiAttemptOutput, { recursive: true });
  await recordPiSecurityWorkerScanDraft(multiAttemptContext, {
    ...workerInput,
    complete: false,
  });
  ...
  const malformedArchivedContext = bindContext(workerContext, malformedArchivedOutput);
  await mkdir(malformedArchivedOutput, { recursive: true });
  await recordPiSecurityWorkerScanDraft(malformedArchivedContext, {
    ...workerInput,
    complete: false,
  });
  ...
  const crossScanRetryContext = bindContext(workerContext, crossScanRetryOutput);
  await recordPiSecurityWorkerScanDraft(crossScanRetryContext, {
    ...workerInput,
    complete: false,
  });
  ...
  await assert.rejects(
    recordPiSecurityWorkerScanDraft(crossScanRetryContext, {
      ...replacementAttempt,
      findings: [],
    }),
  ...
  try {
    await assert.rejects(
      recordPiSecurityWorkerScanDraft(
        bindContext(workerContext, unreadableRetryOutput),
        workerInput,
      ),
  ...
  const partialDeferredContext = bindContext(workerContext, partialDeferredRoot);
  const partialDeferredFinding = { summary: "Partial evidence captured before validation." };
  await recordPiSecurityWorkerScanDraft(partialDeferredContext, {
    ...workerInput,
    complete: false,
    findings: [],
    coverage: {
  ...
  await recordPiSecurityWorkerScanDraft(partialDeferredContext, workerInput);
  ...
  const recorded = await recordPiSecurityScanDraft(context, input);
  ...
  await recordPiSecurityScanDraft(context, semanticInput);
  ...
  await recordPiSecurityScanDraft(context, reasonOnlyInput);
  assert.deepEqual(
  ...
  await assert.rejects(
    recordPiSecurityScanDraft(context, input),
  ...
  const identityContext = bindContext(workerContext, identityRoot);
  await recordPiSecurityWorkerScanDraft(identityContext, {
    ...workerInput,
    findings: [{ ...finding, identity: explicitIdentity }],
  });
  await recordPiSecurityWorkerScanDraft(identityContext, workerInput);
  assert.deepEqual(
  ...
  const diffDigest =
    "pi-security-snapshot/v1:sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";
  const diffContext = {
    ...context,
    mode: "diff",
    targetContract: {
  ...
  for (const kind of ["commit", "range"]) {
    ...
    assert.equal(
      ...
      `pi-security-snapshot/v1:sha256:${expectedDigest}`,
    );
    assert.equal(
  ...
  for (const [description, rejectedInput] of rejectedDraftInputs) {
    await assert.rejects(
      recordPiSecurityScanDraft(context, rejectedInput),
      Error,
      description,
    );
    assert.equal(
  ...
  await assert.rejects(
    recordPiSecurityScanDraft(context, {
      ...input,
  ...
  await assert.rejects(
    recordPiSecurityScanDraft({ ...context, layout: "worker" }, input),
    /authoritative parent scan context/,
  );
  await assert.rejects(
    recordPiSecurityScanDraft({ ...context, status: "complete" }, input),
  ...
  await assert.rejects(
    getPiSecurityCompletedScan(context, {
      scanId,
      handoffClaimToken: claimToken,
    }),
    /has not completed/,
  ...
  assert.deepEqual(
    await getPiSecurityCompletedScan(completeContext, {
      scanId,
      handoffClaimToken: claimToken,
    }),
    {
  ...
  await assert.rejects(
    getPiSecurityCompletedScan(completeContext, {
      scanId,
      handoffClaimToken: claimToken,
    }),
...
console.log("Pi Security scan draft artifact tests passed");
...
async function recordFreshScanDraft(context, input) {
  ...
  return recordPiSecurityScanDraft(context, input);
}
