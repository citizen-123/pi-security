...
def load_script(name: str) -> ModuleType:
    path = PACKAGE_DIR / "scripts" / f"{name}.py"
...
def test_projection_renders_inline_code_and_section_code_evidence() -> None:
    ...
    assert "`pi-rs/exec-server/src/environment.rs:253-281`" in markdown
...
def test_projection_includes_exact_target_identity() -> None:
    ...
    manifest["scan"]["target"].update(
        {
            ...
            "snapshotDigest": "pi-security-snapshot/v1:sha256:" + "a" * 64,
        }
    )
    ...
    assert "- Snapshot digest: pi-security-snapshot/v1:sha256:" in markdown
