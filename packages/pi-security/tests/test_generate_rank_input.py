...
SCRIPT = PACKAGE_ROOT / "scripts" / "generate_rank_input.py"
...
def test_cli_loads_preview_helper_with_safe_path() -> None:
    ...
    assert "Pi Security scan worklist helper" in result.stdout
...
def initialize_repo(repo: Path) -> None:
    git(repo, "init", "-q")
    git(repo, "config", "user.email", "pi-security-tests@example.com")
    git(repo, "config", "user.name", "Pi Security Tests")
...
def test_make_repo_rank_input_combines_explicit_files_and_directories(tmp_path: Path) -> None:
    ...
    (repo / "src").mkdir(parents=True)
    (repo / "tests").mkdir()
    (repo / "~pi_review_nonexistent_user" / "src").mkdir(parents=True)
    (repo / "src" / "runtime.py").write_text("runtime = True", encoding="utf-8")
    (repo / "tests" / "security_test.py").write_text("security = True", encoding="utf-8")
    (repo / "~pi_review_nonexistent_user" / "src" / "literal.py").write_text(
    ...
    scopes.write_text(
        json.dumps(
            [
                ...
                "~pi_review_nonexistent_user/src",
                unicode_file.name,
                "src/runtime.py",
                "Dockerfile",
                "package-lock.json",
    ...
    assert [row["path"] for row in read_jsonl(output)] == sorted(
        [
            "src/runtime.py",
            "tests/security_test.py",
            "~pi_review_nonexistent_user/src/literal.py",
            unicode_file.name,
            "Dockerfile",
            "package-lock.json",
        ]
