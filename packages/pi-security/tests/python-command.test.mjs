...
let environmentProbeCount = 0;
assert.equal(await resolvePythonCommand({
  configuredPython: "/configured/python",
  environment: {
    PI_SECURITY_PYTHON_COMMAND: "  /pi-security/python  ",
    PYTHON: "/python/environment"
  },
  isUsableExecutable: async () => {
    environmentProbeCount += 1;
    return false;
  }
}), "/pi-security/python");
assert.equal(environmentProbeCount, 0);
...
const executableFixtureRoot = await mkdtemp(path.join(tmpdir(), "pi-security-python-command-"));
try {
