...
const bundle = await build({
  ...
  stdin: {
    contents: [
      'export * from "./src/artifact-attack-path.ts";',
...
async function testEligibleRowsKeepDiscoveryValidationAndOrder() {
  ...
  const result = await recordPiSecurityCandidateAttackPaths(fixture.context, {
    attackPaths: [
...
async function testUnknownAndDuplicateCandidatesDoNotChangeLedger() {
  ...
  await assert.rejects(
    recordPiSecurityCandidateAttackPaths(fixture.context, {
...
async function testInvalidAttackPathsDoNotChangeLedger() {
  ...
  for (const value of invalid) {
    ...
    await assert.rejects(recordPiSecurityCandidateAttackPaths(
      fixture.context,
      payload
    ));
    await assertUnchanged(fixture);
...
async function testEmptyLedgerAcceptsAnEmptyBatch() {
  ...
  assert.deepEqual(await recordPiSecurityCandidateAttackPaths(
    fixture.context,
    { attackPaths: [] }
  ), {
    kind: "candidate_attack_paths",
...
async function createFixture(label, originalRows) {
  const root = await realpath(await mkdtemp(path.join(
    tmpdir(),
    `pi-security-attack-path-${label.replace(/\s+/gu, "-")}-`
  )));
  temporaryRoots.push(root);
  const artifactRoot = path.join(root, "scan");
