...
const scanDir = "/tmp/pi-security-contract-fixture";
...
for (const tool of [
  "get_pi_security_scan_context",
  "update_pi_security_scan_progress",
  "complete_pi_security_scan",
  "fail_pi_security_scan"
]) {
  assert.ok(prompt.includes(tool), `handoff prompt must name ${tool}`);
}
assert.equal(
  (promptWithContext.match(/\bget_pi_security_scan_context\b/g) ?? []).length,
  1,
...
for (const operation of [
  "prepare_pi_security_review_items",
  "list_pi_security_review_items",
  "record_pi_security_discovery_candidates",
  "list_pi_security_candidates",
  "record_pi_security_candidate_validations",
  "record_candidate_attack_paths",
  "get_pi_security_completed_scan"
]) {
  assert.equal(
...
for (const mode of ["standard", "diff", "deep"]) {
  ...
  assert.match(
    modePrompt,
    /\bcancel_pi_security_scan\b[^.;]{0,100}\buser explicitly cancels\b/u,
    `${mode} handoff must use scan cancellation for explicit user cancellation`
  );
  assert.doesNotMatch(
    modePrompt,
    /\bfail_pi_security_scan\b[^.;]{0,160}\b(?:user explicitly cancels|explicit cancellation)\b/u,
...
for (const mode of ["standard", "diff", "deep"]) {
  ...
  assert.ok(
    compactPrompt.includes("complete_pi_security_scan"),
    `${mode} handoff must expose scan completion`
  );
  assert.equal(
    compactPrompt.includes("record_pi_security_scan_draft"),
    mode !== "deep",
    `${mode} handoff must assign semantic draft construction to its actual owner`
  );
  assert.equal(
    compactPrompt.includes("get_pi_security_completed_scan"),
    false,
...
assert.match(
  deepPrompt,
  /Leave progress in preflight until start_pi_security_deep_scan begins discovery/
);
...
assert.ok(
  deepPrompt.indexOf("start_pi_security_deep_scan")
    < deepPrompt.indexOf("complete_pi_security_scan"),
