...
import { replaceArtifactJson } from "./artifact-io.js";
import {
...
export async function getPiSecurityDeepReducerInputs(
...
export async function recordPiSecurityDeepReduction(
  context: ArtifactContext,
  input: unknown
): Promise<{
  findingCount: number;
  ...
  return withLogicalReducerErrors(context, async () => {
    ...
    await replaceArtifactJson(context, bound.resultPath, reduction);
    return {
