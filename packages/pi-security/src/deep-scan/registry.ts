...
import { resolveExecutionScanPath } from "../execution-boundary.js";
import { createExecutionPolicyContext } from "../execution-policy.js";
import { PolicyRecoveryRejectedError } from "../enforcement-capabilities.js";
import {
...
export class DeepScanRemoteCoordinator {
  ...
  async wait(
    ...
    if (!threadId) {
      throw new Error("Observing a Deep Scan requires its owning host session.");
    }
    ...
    while (true) {
      ...
      if (
        ...
        try {
          await validateInterruptedWorkerPolicies(run, options);
...
export async function startOrJoinDeepScanCoordinator(input: {
  ...
  if (!threadId) {
    throw new Error("Starting or joining a Deep Scan requires its owning host session.");
  }
  await validateInterruptedWorkerPolicies(input.begin.run, input.options);
...
async function validateInterruptedWorkerPolicies(
  run: DeepScanRunState,
  options: Pick<CoordinatorOptions, "executor" | "packageRoot">,
): Promise<void> {
  await validateResumableWorkerPolicies({
    ...
    resumableWorkers: run.persistedWorkers ?? [],
  }, options.executor, options.packageRoot);
}

export async function validateResumableWorkerPolicies(
  bindings: {
    scanId: string;
    targetPath: string;
    scope: string;
    scanDir: string;
    subagents: number;
    resumableWorkers: readonly PersistedDeepScanWorker[];
  },
  executor: PiWorkerExecutor,
  packageRoot: string,
): Promise<void> {
  const scanContext = createExecutionPolicyContext({
    profile: "security-artifact-writer",
    target: { root: bindings.targetPath },
    scan: { id: bindings.scanId, artifactRoot: bindings.scanDir },
  });
  for (const worker of bindings.resumableWorkers) {
    ...
    try {
      artifactDir = (
        await resolveExecutionScanPath(
          scanContext,
          worker.artifactDir,
          {
            expected: "directory",
            label: "Resumable worker artifact directory",
          },
        )
      ).absolute;
      workerRoot = (
        await resolveExecutionScanPath(
          scanContext,
          dirname(worker.artifactDir),
          {
            expected: "directory",
            label: "Resumable worker root",
          },
        )
      ).absolute;
    } catch (error) {
      throw new PolicyRecoveryRejectedError(
        "binding_mismatch",
        "A resumable Deep Scan worker is outside its authoritative scan root.",
        { cause: error },
      );
    }
    if (dirname(artifactDir) !== workerRoot) {
      throw new PolicyRecoveryRejectedError(
        "binding_mismatch",
        "A resumable Deep Scan worker artifact directory changed identity.",
      );
    }
    const subagents = worker.kind === "discovery" ? bindings.subagents : 0;
    await executor.validateContinuationPolicy({
      ...
      artifactContext: {
        root: artifactDir,
        workerRoot,
        repoRoot: bindings.targetPath,
        scanId: bindings.scanId,
        layout: worker.kind === "dedup" ? "reducer" : "worker",
        scope: bindings.scope,
        packageRoot,
        scanRoot: bindings.scanDir,
      },
      executionContext: issueDeepScanSourceContext({
        targetRoot: bindings.targetPath,
        scanId: bindings.scanId,
        workerRoot,
        delegationBudget: subagents,
      }),
      artifactWriterContext: issueDeepScanArtifactWriterContext({
        targetRoot: bindings.targetPath,
        scanId: bindings.scanId,
