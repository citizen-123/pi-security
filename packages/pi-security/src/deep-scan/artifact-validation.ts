...
import { readJsonObject, requireRegularFile } from "./artifacts.js";
...
export async function validateReducerArtifacts(input: {
  ...
  if (input.sources) {
    ...
    await saveScanDraftCheckpoint(input.artifactContext, result);
    await replaceArtifactJson(input.artifactContext, resultPath, result);
  } else {
