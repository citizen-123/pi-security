...
import {
  aggregateWorkerExecutionDiagnostics,
...
export class DeepScanCoordinator {
  ...
  private async run(): Promise<void> {
    try {
      if (this.state.policyFailure) {
        throw new Error(this.state.policyFailure.message);
      }
      ...
      const policyFailure = describePolicyEnforcementFailure(error) ?? this.state.policyFailure;
      const persistedMessage = policyFailure?.message ?? boundedDeepScanErrorMessage(error);
      if (this.phase === "setup") {
  ...
  private finishLocally(state: DeepScanRunState): void {
    ...
    this.state = {
      ...state,
      diagnostics: aggregateWorkerExecutionDiagnostics(this.audit.executions)
    };
    this.resolveTerminal(cloneState(this.state));
  }
  ...
  private async recoverCompletedReducers(
    ...
    for (const worker of completedReducers) {
      ...
      const executionPolicy = createExecutionPolicyContext({
        ...
        target: { root: this.state.targetPath },
        scan: { id: this.state.scanId, artifactRoot: worker.artifactDir },
      });
      const artifactContext = await createWorkerArtifactContext({
        ...
        deepReducer: {
          ...
          claimedWorkers: accepted.map((discovery) => ({
            id: discovery.id,
            resultPath: discovery.resultPath,
          })),
