...
export async function preparePiSecurityReviewItems(
  ...
  if (!context.packageRoot) {
    throw new Error(`${label}: the scan has no bound package context.`);
  }
  ...
  const helper = join(context.packageRoot, "scripts", "generate_in_scope_files.py");
  const arguments_ = [
    helper,
    "--repo",
    context.repoRoot,
...
export async function listPiSecurityReviewItems(
  context: ArtifactContext,
  page: ArtifactPage = {}
