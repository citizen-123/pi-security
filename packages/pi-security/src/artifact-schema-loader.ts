...
export function bundleArtifactSchema(
  ...
  for (const document of documents) {
    if (documentsById.has(document.$id)) {
      throw new Error("Duplicate Pi Security schema document: " + document.$id);
    }
  ...
  if (!document) {
    throw new Error("Unknown Pi Security schema document: " + documentId);
  }
  const source = document.$defs[definition];
  if (!source || typeof source !== "object" || Array.isArray(source)) {
    throw new Error("Unknown Pi Security schema definition: " + definition);
  }
...
function dereferenceArtifactSchema(
  ...
  if (!referencedDocument) {
    throw new Error("Unknown Pi Security schema reference: " + reference);
  }

  const referenceKey = referencedDocument.$id + "#" + pointer;
  if (activeReferences.has(referenceKey)) {
    throw new Error("Cyclic Pi Security schema reference: " + reference);
  }
...
function readSchemaPointer(document: SchemaDocument, pointer: string): unknown {
  ...
  if (!pointer.startsWith("/")) {
    throw new Error("Invalid Pi Security schema pointer: #" + pointer);
  }
  ...
  for (const encoded of pointer.slice(1).split("/")) {
    ...
    if (
      ...
      throw new Error("Unknown Pi Security schema pointer: #" + pointer);
    }
