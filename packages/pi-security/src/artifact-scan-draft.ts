...
export async function recordPiSecurityScanDraft(
  ...
  for (;;) {
    ...
    try {
      ...
      if (publishDraft) {
        ...
        await replaceArtifactJson(context, destinations[0], { findings });
        await replaceArtifactJson(context, destinations[1], coverage);
        await replaceArtifactJson(context, destinations[2], { scan: manifestScan });
      }
      return {
...
export async function recordPiSecurityScanDraftViaWorkbench(
  context: ArtifactContext,
  input: ScanDraftInput,
  runWorkbench: RunArtifactWorkbench,
  signal?: AbortSignal,
): Promise<ScanDraftResult> {
  return recordPiSecurityScanDraft(
    context,
    input,
    async (draft, expectedDigest, checkpoint) => {
      ...
      try {
        ...
        await Promise.all([
          replaceArtifactJson(context, checkpointPath, snapshot),
          replaceArtifactJson(context, draftPath, draft),
        ]);
        const arguments_ = [
          "write-scan-draft",
          "--scan-id",
...
export async function recordPiSecurityWorkerScanDraft(
  context: ArtifactContext,
  input: ScanDraftInput,
): Promise<ScanDraftResult> {
  const parsed = parseScanDraft(input);
  ...
  await replaceArtifactJson(context, destination, scoped);
...
export async function saveScanDraftCheckpoint(
  ...
  if (context.layout === "worker" && updateHead) {
    ...
    await replaceArtifactJson(context, head, { checkpoint: name });
  }
}
...
export async function getPiSecurityCompletedScan(
  context: ArtifactContext,
  ...
  if (context.mode === "diff") {
    ...
    if (diffTarget.kind === "working_tree") {
      ...
      const digest = createHash("sha256")
        ...
        .digest("hex");
      target.snapshotDigest = `pi-security-snapshot/v1:sha256:${digest}`;
    } else {
      throw new Error(
