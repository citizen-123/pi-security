...
After preserving any continuation scan context and applying its one-time concurrent-scan guard, read `../../references/scan-prologue.md` once. Deep scans do not request provider credentials or configuration remediation. The coordinator validates ownership, target, and scope, then uses standard MCP sampling for independent workers. If the connected MCP host does not advertise sampling, stop with the returned capability error and use a Standard scan instead.
