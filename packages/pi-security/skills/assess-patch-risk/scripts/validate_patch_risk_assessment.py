...
SCHEMA_PATH = PACKAGE_ROOT / "schemas" / "patch-risk-assessment.schema.json"
NON_APPLICABLE = {"no_live_effect", "wrong_owner", "duplicate", "superseded"}


def load_scan_contract_validator() -> ModuleType:
    script = PACKAGE_ROOT / "scripts" / "finalize_scan_contract.py"
    spec = importlib.util.spec_from_file_location("pi_security_scan_contract", script)
