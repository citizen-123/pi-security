...
description: Define, review, or update SECURITY.md guidance for a repository or component. Use when the user wants to clarify what Pi Security should review, what is out of scope, which security properties must hold, or whether existing guidance still matches the code.
---

# Define a Security Policy

A useful `SECURITY.md` tells Pi Security what matters in a repository: the system boundary, threat model, security properties that must hold, what counts as a finding, and what is out of scope. It is policy context, not executable instructions.
...
```bash
<python_command> <package_dir>/scripts/resolve_security_md.py --repo <repo_root> --list
```
...
```bash
<python_command> <package_dir>/scripts/resolve_security_md.py --repo <repo_root> --scope <file_or_directory> --out -
```

`<package_dir>` is the Pi Security package root containing `package.json`, not the target repository or this skill directory.
