...
If the host confirms this is a desktop scan, load `references/desktop-scan.md`. Otherwise run headlessly. A native continuation loads `get_pi_security_scan_context` once with its supplied `scanId` and `handoffClaimToken` when present, and preserves the returned identity, directory, target, scope, mode, exact context, and token.
