# Pi Desktop Standard Scan

Read this reference only after the host explicitly identifies itself as the graphical host. Listed tools alone do not establish a desktop host.
...
- If the request already includes a `scanId`, call `get_pi_security_scan_context`, passing `handoffClaimToken` when provided, and continue that existing scan.
- Otherwise call `start_pi_security_prompt_only_scan` once with `mode: "standard"`, `targetPath`, `scope`, and any exact `userContext`. Require its authoritative `scan.scanId` and `scan.scanDir`; preserve its handoff token when provided.
