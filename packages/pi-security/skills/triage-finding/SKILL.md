...
That schema describes completed Pi Security scan output. It requires generated fields such as `scanId`, `findingId`, `occurrenceId`, fingerprints,
...
Use the schema only as an optional compatibility source when the user supplies an existing `pi-security.findings` JSON artifact. In that case, extract the available fields into the triage normalization record and preserve the original IDs as source identifiers. The triage result contract is defined in `references/triage-result-contract.md`.
...
the current Pi project's attached GitHub repository, or the current local repository's GitHub remote.

If the user asks to pull from GitHub without typing an `owner/repo` or URL, first infer the GitHub repository from the current Pi project attachment when that metadata is available. Prefer that attached repository over a local path or local git remote. If no Pi project attachment is visible, fall back to the current repository's GitHub remote. Only ask for a repository URL or `owner/repo`
...
SARIF results, CVE/GHSA or advisory descriptions, scanner tickets, bug bounty report snippets, Jira/Linear issue URLs or searches, Pi Security finding artifacts, or a freeform vulnerability claim. If useful, ask for the repository path or affected file/component at the same time.
...
- input id, scanner id, SARIF rule/result id, CVE/GHSA id, ticket id, or Pi Security `findingId`/`occurrenceId` when present
- title or short claim
- source type: `sarif`, `cve`, `advisory`, `scanner_ticket`,
  `bug_bounty`, `pi_security_finding`, `freeform`, or `unknown`
...
3. Normalize each supplied or imported finding into a triage item.
   ...
   - Do not invent scanner fields, generated Pi Security ids, severity, or remediation just to satisfy another schema.
