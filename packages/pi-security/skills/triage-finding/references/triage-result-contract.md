...
That schema is for completed Pi Security scan artifacts. It requires scan metadata, generated finding ids, occurrence ids, fingerprints, severity,
...
If the user supplies a valid `pi-security.findings` artifact, use it as an input source and map its fields into this triage contract. Preserve `findingId`, `occurrenceId`, and `ruleId` as source identifiers; do not require new triage inputs to conform to the completed-scan schema.
