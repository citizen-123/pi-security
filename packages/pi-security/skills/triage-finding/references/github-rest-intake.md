...
- the current Pi project's attached GitHub repository
...
1. Current Pi project attached GitHub repository, when visible in the project context.
2. Current local repository's GitHub remote.
3. Explicit follow-up asking for `owner/repo` or a GitHub repository URL.

Prefer the Pi project attachment over a local path or local git remote. In the source-selection prompt, name the inferred `owner/repo` so the user can see which repository will be queried.
