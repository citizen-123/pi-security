#!/usr/bin/env python3
"""Pi Security workbench persistence."""
...
READ_ONLY_PREFLIGHT_ENV = "PI_SECURITY_WORKBENCH_READ_ONLY_PREFLIGHT"
READ_ONLY_PREFLIGHT_COMMANDS = frozenset({"get-workspace", "preflight-deep-scan"})
READ_ONLY_PREFLIGHT_NOT_FOUND_EXIT = 66
READ_ONLY_PREFLIGHT_NOT_FOUND_CODE = "PI_SECURITY_WORKBENCH_STATE_NOT_FOUND"
WORKBENCH_SETUP_CHANGED_EXIT = 75
WORKBENCH_SETUP_CHANGED_CODE = "PI_SECURITY_SETUP_CHANGED"


class ReadOnlyWorkbenchStateNotFoundError(Exception):
    pass


class WorkbenchSetupChangedError(Exception):
    pass
...
def state_dir() -> Path:
    state_dir = os.environ.get("PI_SECURITY_STATE_DIR")
    if state_dir:
        return Path(state_dir).expanduser().resolve()
    pi_home = Path(os.environ.get("PI_HOME", "~/.pi")).expanduser()
    return (pi_home / "security").resolve()
...
def connect_read_only_preflight() -> sqlite3.Connection:
    path = database_path()
    if not path.is_file():
        raise ReadOnlyWorkbenchStateNotFoundError
    ...
    try:
        ...
        if query_only is None or query_only[0] != 1:
            raise RuntimeError("SQLite read-only preflight could not enable query-only mode.")
        return connection
...
def require_scannable_target(target: Path) -> None:
    ...
    if metadata["isGit"] and not metadata["isWorktree"]:
        raise SystemExit(
            "Pi Security requires a checked-out worktree, not a bare Git repository."
        )
...
def workbench_completion_binding(scan: sqlite3.Row, completed_at: str) -> dict[str, Any]:
    ...
    package_manifest = read_json_object(
        Path(__file__).resolve().parent.parent / "package.json"
    )
    package_version = package_manifest.get("version")
    if not isinstance(package_version, str) or not package_version:
        raise SystemExit("package.json: expected a nonempty Pi Security package version.")
    target: dict[str, Any] = {
    ...
    return {
        ...
        "producer": {"name": PRODUCER_NAME, "version": package_version},
        "target": target,
...
def require_workspace(connection: sqlite3.Connection, workspace_id: str) -> sqlite3.Row:
    ...
    if row is None:
        raise SystemExit("Pi Security workspace not found. Reopen it to continue.")
    return row
...
def resolve_scan_id(connection: sqlite3.Connection, scan_id: str) -> str:
    try:
        ...
        if not matches:
            raise SystemExit("Pi Security scan not found.") from None
        if len(matches) > 1:
            raise SystemExit(
...
def require_start_scan_preflight_binding(
    workspace: sqlite3.Row, args: argparse.Namespace
) -> None:
    expected_mode = getattr(args, "expected_mode", None)
    expected_target_path = getattr(args, "expected_target_path", None)
    if (expected_mode is None) != (expected_target_path is None):
        raise SystemExit(
            "Start Scan expected-mode and expected-target-path must be provided together."
        )
    if expected_mode is None:
        return
    if (
        workspace["default_mode"] != expected_mode
        or workspace["target_path"] != expected_target_path
    ):
        raise WorkbenchSetupChangedError(
            "Pi Security setup changed after enforcement preflight. "
            "Retry from the current setup."
        )
...
def start_scan(connection: sqlite3.Connection, args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        require_start_scan_preflight_binding(workspace, args)
        ...
        if not workspace["submitted"] or not workspace["target_path"]:
            raise SystemExit("Save the Pi Security setup before starting the scan.")
        ...
        if workspace["updated_at"] != workspace_version:
            raise WorkbenchSetupChangedError(
                "Pi Security setup changed while the scan was starting. "
                "Retry from the current setup."
            )
        ...
        if (current_target_metadata.st_dev, current_target_metadata.st_ino) != (
            ...
            raise WorkbenchSetupChangedError(
                "The selected scan target changed while the scan was starting. "
                "Retry from the current setup."
            )
        if workspace["default_mode"] == "deep" and workspace["thread_id"] is not None:
            ...
            if owned_active_scan is not None:
                raise SystemExit(
                    "This host session already has an active Deep Scan for the selected "
                    "target and scope. Rejoin that scan instead of starting another one."
                )
        create_scan_target_root(args, target_root)
        insert_running_scan(
            connection,
            scan_id=scan_id,
            workspace=workspace,
...
def request_finding_remediation_action(
    ...
    try:
        ...
        if current is None or current["occurrence_id"] != occurrence["id"]:
            raise SystemExit("Pi Security finding remediation request not found.")
...
def require_reviewed_patch_applied(
    ...
    with tempfile.TemporaryDirectory(prefix="pi-security-remediation-") as temporary:
...
def read_only_workspace_preflight_state(
    connection: sqlite3.Connection,
    workspace_id: str,
    *,
    thread_id: str | None = None,
) -> dict[str, Any]:
    normalized_workspace_id = require_uuid(workspace_id, "workspace-id")
    workspace = connection.execute(
        """
        SELECT id, target_path, default_mode, thread_id
        FROM workspaces
        WHERE id = ?
        """,
    ...
    if workspace is None:
        raise SystemExit("Pi Security workspace not found. Reopen it to continue.")
    if thread_id is not None and workspace["thread_id"] != optional_text(
        thread_id, maximum=512
    ):
        raise SystemExit("Pi Security workspace not found in this thread.")
    return {
        "id": workspace["id"],
...
def run_read_only_preflight(args: argparse.Namespace) -> dict[str, Any]:
    if args.command not in READ_ONLY_PREFLIGHT_COMMANDS:
        raise SystemExit(
            "Read-only workbench preflight only supports get-workspace and "
            "preflight-deep-scan."
        )
    try:
        connection = connect_read_only_preflight()
    except ReadOnlyWorkbenchStateNotFoundError:
        print(
            json.dumps(
                {
                    "schemaVersion": 1,
                    "code": READ_ONLY_PREFLIGHT_NOT_FOUND_CODE,
        ...
        raise SystemExit(READ_ONLY_PREFLIGHT_NOT_FOUND_EXIT) from None
    with closing(connection):
        if args.command == "get-workspace":
            return read_only_workspace_preflight_state(
                connection,
                args.workspace_id,
                thread_id=args.thread_id,
            )
        return deep_scan.preflight_deep_scan_read_only(connection, args)
...
def main() -> None:
    ...
    read_only_preflight = os.environ.get(READ_ONLY_PREFLIGHT_ENV)
    if read_only_preflight is not None:
        if read_only_preflight != "1":
            raise SystemExit(f"{READ_ONLY_PREFLIGHT_ENV} must be 1 when set.")
        result = run_read_only_preflight(args)
    ...
    with closing(connect()) as connection:
        ...
        elif args.command == "preflight-deep-scan":
            result = deep_scan.preflight_deep_scan(connection, args)
...
if __name__ == "__main__":
    try:
        main()
    except WorkbenchSetupChangedError as error:
        print(
            json.dumps(
                {
                    "schemaVersion": 1,
                    "code": WORKBENCH_SETUP_CHANGED_CODE,
        ...
        raise SystemExit(WORKBENCH_SETUP_CHANGED_EXIT) from None
