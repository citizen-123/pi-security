#!/usr/bin/env python3
"""Validate and seal additive Pi Security scan-contract artifacts."""
...
SCHEMA_VERSION = "1.0"
PRODUCER_NAME = "pi-security"
FINGERPRINT_ALGORITHM = "pi-security/v1"
...
def _generate_report_projection(
    ...
    spec = importlib.util.spec_from_file_location("pi_security_report_projection", script)
...
def _windows_scan_local_files() -> Any:
    ...
    if _WINDOWS_SCAN_LOCAL_FILES is None:
        ...
        spec = importlib.util.spec_from_file_location("pi_security_windows_scan_files", script)
        if spec is None or spec.loader is None:
...
def write_external_output_bytes(output: Path, payload: bytes) -> None:
    ...
    write_scan_local_bytes(parent, relative_output, payload, external_name=True)
...
def _populate_unsealed_manifest_envelope(
    ...
    manifest["documentType"] = "pi-security.scan-manifest"
    ...
    if completion_binding is None:
        started_at = os.environ.get("PI_SECURITY_STARTED_AT")
        if started_at is not None:
            _validate_date_time(started_at, "PI_SECURITY_STARTED_AT")
...
def _populate_unsealed_artifact_envelope(
    ...
    findings["documentType"] = "pi-security.findings"
    findings["schemaVersion"] = SCHEMA_VERSION
    coverage["documentType"] = "pi-security.coverage"
...
def _validate_manifest(manifest: dict[str, Any]) -> None:
    if manifest.get("documentType") != "pi-security.scan-manifest":
        raise ContractError("manifest.documentType: expected pi-security.scan-manifest")
...
def _validate_findings(manifest: dict[str, Any], findings: dict[str, Any]) -> None:
    if findings.get("documentType") != "pi-security.findings":
        raise ContractError("findings.documentType: expected pi-security.findings")
...
def _sarif_result(
    ...
    partial_fingerprints = {
        "piSecurity/v1": finding["fingerprints"]["primary"],
    }
...
def build_sarif(
    ...
    run: dict[str, Any] = {
        ...
        "properties": {
            "piSecuritySchemaVersion": manifest["schemaVersion"],
            "piSecurityTargetKind": target["kind"],
        },
    }
...
def build_sarif_projection(
    ...
    if not execution_successful or coverage["completeness"] != "complete":
        run = sarif["runs"][0]
        run["properties"]["piSecurityCoverageCompleteness"] = coverage["completeness"]
        run["invocations"] = [
            {
...
def _write_sarif_projection_if_possible(
    ...
    try:
        ...
        print(
            f"pi-security: warning: automatic SARIF export failed: {error}. "
