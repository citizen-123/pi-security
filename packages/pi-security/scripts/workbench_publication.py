...
def verify_linear_publication_scan(
    ...
    try:
        ...
        raise SystemExit(
            "The completed scan is not present in the local Pi Security scan-history database. "
    ...
    if os.path.normcase(requested_directory) != os.path.normcase(recorded_directory):
        raise SystemExit(
            "The selected scan directory does not match its local Pi Security scan history."
        )
    ...
    for finding in findings:
        if stored_findings.get(finding["occurrenceId"]) != finding["findingId"]:
            raise SystemExit(
                ...
                "in local Pi Security scan history."
            )
    if len(stored_findings) != len(findings):
        raise SystemExit(
            "The completed scan findings do not exactly match local Pi Security scan history."
        )
    return scan
...
def export_findings(
    ...
    if path is None:
        raise SystemExit(f"Could not export Pi Security findings as {args.format.upper()}.")
    return {
