"""Measure scan-owned token usage from an optional harness session database."""
...
def reconcile_completed_scan_cost(
    ...
    """Persist authoritative host-reported cost without discarding measured worker usage."""
...
def collect_scan_usage(
    ...
    *,
    ...
    state_database = _session_state_database()
    if state_database is None:
        return _unavailable_usage("session_state_unavailable")
...
def _session_state_database() -> Path | None:
    configured_database = os.environ.get("PI_SECURITY_SESSION_DB", "").strip()
...
def _require_state_columns(
    ...
    if not required.issubset(columns):
        raise ValueError("Pi state graph does not expose the required thread columns.")
