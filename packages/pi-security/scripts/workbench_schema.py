"""SQLite schema history for the Pi Security workbench."""
...
MIGRATIONS = (
    ...
    (
        ...
        ALTER TABLE finding_workflow_reviews ADD COLUMN pi_version TEXT;
    ...
    (
        40,
        "persist deep scan policy failure identity",
        """
        ALTER TABLE deep_scan_runs ADD COLUMN policy_failure_code TEXT;
        ALTER TABLE deep_scan_runs ADD COLUMN policy_failure_category TEXT;
        ALTER TABLE deep_scan_runs ADD COLUMN policy_failure_reason TEXT;
...
def migrate_finding_workflow_review_columns(connection: sqlite3.Connection) -> None:
    for row in connection.execute(
        ...
        scope = binding["scope"]
        connection.execute(
            """UPDATE finding_workflow_reviews SET review_contract_version = ?, pi_version = ?,
...
def normalize_pre_release_execution_profile_migrations(
    ...
    if any(
        ...
        raise SystemExit(
            "The Pi Security database has an unsupported execution-profile migration history."
        )
...
def normalize_pre_release_migrations(connection: sqlite3.Connection, timestamp: str) -> None:
    ...
    if (
        ...
        if (
            ...
            raise SystemExit(
                "The Pi Security database has an unsupported pre-release migration history."
            )
        connection.execute(
...
def normalize_mirror_lineage_migrations(connection: sqlite3.Connection) -> None:
    ...
    if migrations != mirror_names:
        raise SystemExit("The Pi Security database has an unsupported mirror migration history.")
