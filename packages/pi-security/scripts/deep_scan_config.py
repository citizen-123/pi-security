"""Resolve per-user Pi Security Deep Scan orchestration settings."""
...
def pi_home() -> Path:
    return Path(os.environ.get("PI_HOME", "~/.pi")).expanduser()


def config_path() -> Path:
    configured = os.environ.get("PI_SECURITY_DEEP_SCAN_CONFIG_PATH", "").strip()
    if configured:
        return Path(configured).expanduser()
    return pi_home() / "pi-security" / "config.toml"
...
def resolve_deep_scan_config(available_parallelism: int) -> dict[str, int | float]:
    ...
    if path.exists():
        try:
            ...
            raise SystemExit(f"Cannot read Pi Security configuration at {path}: {exc}") from exc
        if not isinstance(document, dict):
            raise SystemExit(f"Pi Security configuration at {path} must be a TOML table.")
        deep_scan: object = document.get("deep_scan", {})
        if not isinstance(deep_scan, dict):
            raise SystemExit(
                f"Pi Security configuration [deep_scan] at {path} must be a TOML table."
            )
        unknown = sorted(set(deep_scan) - CONFIG_KEYS)
        if unknown:
            raise SystemExit(
                f"Unknown Pi Security Deep Scan configuration {', '.join(unknown)} in {path}."
            )
