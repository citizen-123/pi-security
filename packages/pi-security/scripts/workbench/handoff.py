"""Scan handoff state transitions for the Pi Security workbench."""
...
def validate_handoff_delivery_thread(
    ...
    if owning_thread_id != requesting_thread_id and not claim_token.startswith(
        ...
        raise SystemExit(
            "A scan handoff can only be marked delivered from its owning host session."
        )
...
def attach_scan_continuation_thread(
    ...
    *,
    ...
    if thread_id is None:
        raise SystemExit("Pi Security continuation thread ID is required.")
    connection.execute("BEGIN IMMEDIATE")
    try:
        timestamp = now()
        scan = require_scan(connection, scan_id)
        if scan["handoff_claim_token"] != claim_token:
            raise SystemExit("Pi Security continuation thread claim token does not match.")
        if scan["continuation_thread_id"] is not None:
            if scan["continuation_thread_id"] != thread_id:
                raise SystemExit(
                    "Pi Security scan continuation is owned by another continuation."
                )
            connection.commit()
            return workspace_state(connection, scan["workspace_id"])
        updated = connection.execute(
        ...
        if updated.rowcount != 1:
            raise SystemExit("Pi Security continuation thread could not be attached.")
        connection.commit()
    ...
    *,
    ...
    try:
        ...
        if scan["handoff_status"] == "delivered":
            if scan["handoff_claim_token"] != claim_token:
                raise SystemExit(
                    "Pi Security handoff delivery is owned by another continuation."
                )
            connection.commit()
            return workspace_state(connection, scan["workspace_id"])
        updated = connection.execute(
        ...
        if updated.rowcount != 1:
            raise SystemExit("Pi Security handoff delivery could not be recorded.")
        if scan["mode"] != "deep":
            connection.execute(
                """
