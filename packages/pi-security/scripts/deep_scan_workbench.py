"""Persist deterministic Pi Security Deep Scan orchestration state."""
...
def add_policy_failure_arguments(parser: Any) -> None:
    parser.add_argument(
        "--policy-failure-code",
        choices=(
    ...
    parser.add_argument(
        ...
        choices=("unsupported_enforcement", "policy_recovery_rejected"),
    )
    parser.add_argument(
    ...
    parser.add_argument("--policy-failure-message")
...
def register_subcommands(subparsers: Any, positive_int: Callable[[str], int]) -> None:
    ...
    preflight_deep_scan = subparsers.add_parser("preflight-deep-scan")
    preflight_deep_scan.add_argument("--scan-id", required=True)
    preflight_deep_scan.add_argument("--thread-id", required=True)
    preflight_deep_scan.add_argument("--claim-token")
    ...
    add_policy_failure_arguments(upsert_deep_worker)
    ...
    add_policy_failure_arguments(fail_deep_scan)
...
def deep_scan_policy_failure(run: sqlite3.Row) -> dict[str, Any] | None:
    code = run["policy_failure_code"]
    category = run["policy_failure_category"]
    reason = run["policy_failure_reason"]
    message = run["policy_failure_message"]
    if code is None and category is None and reason is None and message is None:
        return None
    if (
        code == "PI_SECURITY_ENFORCEMENT_UNSUPPORTED"
        and category == "unsupported_enforcement"
        and reason is None
    ...
    if (
        ...
        and category == "policy_recovery_rejected"
        and reason
        in {
            "legacy_continuation",
            "profile_mismatch",
    ...
    raise SystemExit("Persisted Deep Scan policy failure identity is invalid.")


def policy_failure_columns(args: argparse.Namespace) -> tuple[str | None, ...]:
    code = args.policy_failure_code
    category = args.policy_failure_category
    reason = args.policy_failure_reason
    message = optional_text(args.policy_failure_message, maximum=2400)
    if code is None and category is None and reason is None and message is None:
    ...
    if (
        ...
        return (code, category, None, message)
    if (
        code == "PI_SECURITY_POLICY_RECOVERY_REJECTED"
        and category == "policy_recovery_rejected"
        ...
        return (code, category, reason, message)
    raise SystemExit("Deep Scan policy failure identity fields are incomplete or inconsistent.")
...
def require_deep_scan_run(connection: sqlite3.Connection, scan_id: str) -> sqlite3.Row:
    ...
    if row is None:
        raise SystemExit("Pi Security Deep Scan orchestration state not found.")
    return row
...
def require_owned_scan(
    ...
    if persisted_owner != owner:
        raise SystemExit("A scan can only be orchestrated from its owning host session.")
...
def deep_scan_state(connection: sqlite3.Connection, scan_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "policyFailure": deep_scan_policy_failure(run),
...
def deep_scan_result(
    ...
    *,
    ...
    artifact_write_authorization: str | None = None,
) -> dict[str, Any]:
    state = deep_scan_state(connection, scan_id)
    result: dict[str, Any] = {"deepScan": state}
    if start_disposition is not None:
        result["startDisposition"] = start_disposition
    if artifact_write_authorization is not None:
        if (
            artifact_write_authorization == "owning_thread_live_rejoin"
            and state["status"] != "running"
        ):
            artifact_write_authorization = "none"
        result["artifactWriteAuthorization"] = artifact_write_authorization
    return result
...
def begin_deep_scan_for_scan(
    ...
    return deep_scan_result(
        ...
        artifact_write_authorization=(
            "creator"
            if disposition == "created"
            else "owning_thread_live_rejoin"
        ),
    )
...
def begin_deep_scan_for_target(
    ...
    try:
        ...
        if existing is not None:
            ...
            connection.commit()
            disposition = "joined" if existing_run is not None else "created"
            return deep_scan_result(
                connection,
                existing["id"],
                start_disposition=disposition,
    ...
    return deep_scan_result(
        connection,
        scan_id,
        start_disposition="created",
        artifact_write_authorization="creator",
    )
...
def deep_scan_preflight_payload(
    connection: sqlite3.Connection, scan: sqlite3.Row
) -> dict[str, Any]:
    run = connection.execute(
        "SELECT subagents FROM deep_scan_runs WHERE scan_id = ?",
        (scan["id"],),
    ).fetchone()
    workers = []
    if run is not None:
        workers = [
            deep_scan_worker_state(worker)
    ...
    return {
        ...
        "scope": scan["scope"],
        "scanDir": scan["scan_dir"],
        "subagents": int(run["subagents"]) if run is not None else 0,
        "resumableWorkers": workers,
    }


def preflight_deep_scan_read_only(
    ...
    scan = connection.execute(
        """
        SELECT scans.id, scans.target_path, scans.scope, scans.scan_dir,
            scans.deep_scan_owner_thread_id, scans.mode, scans.status,
            scans.recipe_json, scans.handoff_status, scans.handoff_claim_token,
            workspaces.thread_id AS workspace_thread_id
        FROM scans
        JOIN workspaces ON workspaces.id = scans.workspace_id
        WHERE scans.id = ?
        """,
    ...
    if scan is None:
        raise SystemExit("Pi Security scan not found.")
    persisted_owner = scan["deep_scan_owner_thread_id"] or scan["workspace_thread_id"]
    claimable_handoff = (
        scan["mode"] == "deep"
        and scan["status"] == "running"
        and scan["recipe_json"] is not None
        and scan["handoff_status"] == "delivered"
        and scan["deep_scan_owner_thread_id"] is None
        and scan["workspace_thread_id"] is None
    )
    if persisted_owner != thread_id and not claimable_handoff:
        raise SystemExit("A scan can only be orchestrated from its owning host session.")
    require_current_continuation(
        scan,
        args.claim_token,
    ...
    if scan["mode"] != "deep":
        raise SystemExit("Deep Scan orchestration requires a scan in deep mode.")
    return {"deepScanPreflight": deep_scan_preflight_payload(connection, scan)}


def preflight_deep_scan(
    connection: sqlite3.Connection, args: argparse.Namespace
) -> dict[str, Any]:
    scan_id = require_uuid(args.scan_id, "scan-id")
    thread_id = optional_text(args.thread_id, maximum=512)
...
def require_deep_scan_worker(connection: sqlite3.Connection, worker_id: str) -> sqlite3.Row:
    ...
    if row is None:
        raise SystemExit("Pi Security Deep Scan worker not found.")
    return row
...
def upsert_deep_scan_worker(
    ...
    try:
        ...
        has_policy_failure = policy_failure[0] is not None
        if has_policy_failure and (
            args.status != "failed"
            or existing is None
            or optional_text(args.error_message, maximum=2400) is None
        ):
            raise SystemExit(
                "A Deep Scan policy failure identity requires an existing failed worker "
                "and an error message."
            )
        if has_policy_failure:
            persisted_policy_failure = (
                run["policy_failure_code"],
                run["policy_failure_category"],
                run["policy_failure_reason"],
                run["policy_failure_message"],
            )
            if any(value is not None for value in persisted_policy_failure):
                if persisted_policy_failure != policy_failure:
                    raise SystemExit("Deep Scan policy failure identity is immutable.")
            else:
                if run["status"] != "running":
                    raise SystemExit(
                        "A completed Deep Scan cannot acquire a policy failure identity."
                    )
                connection.execute(
                    """
                    UPDATE deep_scan_runs
                    SET policy_failure_code = ?, policy_failure_category = ?,
                        policy_failure_reason = ?, policy_failure_message = ?,
                        updated_at = ?
                    WHERE scan_id = ? AND status = 'running'
                    """,
                    (*policy_failure, timestamp, scan_id),
                )
        if existing is None:
            if args.kind == "dedup":
...
def fail_deep_scan_locked(
    ...
    try:
        ...
        if any(value is not None for value in persisted_policy_failure):
            deep_scan_policy_failure(run)
            if all(value is None for value in policy_failure):
                policy_failure = persisted_policy_failure
                message = persisted_policy_failure[3]
            elif policy_failure != persisted_policy_failure:
                raise SystemExit("Deep Scan policy failure identity is immutable.")
        require_current_coordinator(run, args)
        scan = require_scan(connection, scan_id)
        scan_dir = Path(scan["scan_dir"])
        ...
        connection.execute(
            ...
            SET status = ?, phase = 'terminal', cancel_requested = 1,
                error_message = ?, manifest_path = ?,
                policy_failure_code = ?, policy_failure_category = ?,
                policy_failure_reason = ?, policy_failure_message = ?,
                completed_at = ?, updated_at = ?
            WHERE scan_id = ?
            """,
