"""Scan history projection for the native Pi Security workbench."""
...
def list_scans(
    ...
    if os.name == "nt":
        connection.create_function("pi_security_path_key", 1, _windows_path_key)
    clauses: list[str] = []
    values: list[Any] = []
    ...
    if args is not None and args.scan_root:
        ...
        if os.name == "nt":
            ...
            prefix = scan_root.rstrip(os.sep) + os.sep
            clauses.append(
                "(pi_security_path_key(scans.scan_dir) = ? "
                "OR substr(pi_security_path_key(scans.scan_dir), 1, ?) = ?)"
            )
        else:
    ...
    if cached is None and require_matches:
        ...
        if reversed_comparison is not None:
            raise SystemExit(
                ...
                f"'pi-security scans compare {after['id']} {before['id']}'."
            )
        raise SystemExit(
            "No saved matches for these scans. Run 'pi-security scans match BEFORE AFTER' first."
        )
