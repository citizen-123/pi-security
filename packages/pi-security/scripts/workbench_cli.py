"""Command-line argument parsing for the Pi Security workbench."""
...
def parse_args(description: str) -> argparse.Namespace:
    ...
    start_scan.add_argument("--expected-mode", choices=MODES)
    start_scan.add_argument("--expected-target-path")
    start_scan.add_argument("--private-scan-root", action="store_true")
    ...
    runtime_start_attempt = subparsers.add_parser("runtime-start-attempt")
    runtime_start_attempt.set_defaults(runtime_payload_stdin=True)
    runtime_update_attempt = subparsers.add_parser("runtime-update-attempt")
    runtime_update_attempt.set_defaults(runtime_payload_stdin=True)
    ...
    runtime_get_agent = subparsers.add_parser("runtime-get-agent")
    runtime_get_agent.add_argument("--run-id", required=True)
    runtime_get_agent.add_argument("--logical-agent-id", required=True)
