...
SARIF is a deterministic export, not the Pi Security source of truth.
...
- preserves the semantic fingerprint under `piSecurity/v1`
...
SARIF `executionSuccessful` reflects whether the scan reached a successful terminal status, not whether coverage is complete. Incomplete scans retain findings, set `piSecurityCoverageCompleteness`, and include warnings. Integrations must read the coverage file referenced by `scan.coverageRef` before treating the results as exhaustive.
