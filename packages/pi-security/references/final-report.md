# Final Report and Pi Review Directives

Use this guidance when authoring canonical report semantics and returning the generated Pi Security report and review directives.
...
During a host-backed scan, checkpoint saved findings and pending candidates with `complete: false`; a checkpoint is not a completed audit. After a final workbench-owned Standard or workbench-backed diff draft is accepted with `complete: true` (or the backwards-compatible omitted flag), or the Deep coordinator returns its parent scan's canonical manifest, call `complete_pi_security_scan({ scanId, handoffClaimToken? })` and use its returned completion metadata. An externally managed scan returns its unsealed canonical files without calling a completion tool or finalizer; the external host owns completion and report generation. Read full canonical results only when explicitly requested. For a terminal/chat workflow without a `scanId` or completion tool, retain `python <package_dir>/scripts/finalize_scan_contract.py --scan-dir <scan_dir> --source-root <repo_root>` after writing the canonical JSON. Outside the externally managed path, do not mark the scan goal complete until finalization succeeds and the generated report exists.
...
For a whole-repository Deep scan, the workbench derives `coverage.inventoryStrategy: "repository"` in the stored coverage document; repeated discovery is workflow metadata, not a different inventory strategy. Do not include `inventoryStrategy` in `record_pi_security_scan_draft`.
...
## Pi Review Directives

For Pi app rendering, emit one `::code-comment{...}` directive per surviving finding in the final response. The markdown report and review directives should agree on title, file, line range, and core explanation.

Map the final report severity to Pi directive priority only when emitting the directive:
...
For each reportable finding, emit a Pi review directive in this form:
...
- `title` should include the mapped Pi directive priority, formatted like `[P1] Example title`
- `file` should be an absolute path
- `start` and `end` should be tight 1-based line numbers
- `priority` should match the mapped Pi directive priority
